...
const type_default_name: Array<[string, string, string | undefined, boolean]> = [
  ...
  ['date', '2020-01-01', undefined, true],
...
describe('Generate binding type set', async () => {
  const generate_type_option = async (type_str: string, default_value: string, name?: string) => {
    const generate_test = (type_str: string, default_value: string, postfix: string): string =>
      `/* DO NOT EDIT, GENERATED FILE */
archetype type_set_${postfix}

variable res : set<${type_str}> = []

entry set_value(i : set<${type_str}>) {
  res := i
}
`
    await generate_type_gen('set', generate_test, type_str, default_value, name);
  }
  iterate_on_comparable_types(generate_type_option)
})
...
describe('Generate binding type list', async () => {
  const generate_type_option = async (type_str: string, default_value: string, name?: string) => {
    const generate_test = (type_str: string, default_value: string, postfix: string): string =>
      `/* DO NOT EDIT, GENERATED FILE */
archetype type_list_${postfix}

variable res : list<${type_str}> = []

entry set_value(i : list<${type_str}>) {
  res := i
}
`
    await generate_type_gen('set', generate_test, type_str, default_value, name);
