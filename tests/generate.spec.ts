...
const type_default_name: Array<item> = [
  ...
  new item('signature', '"edsigte5U54Z8kxengKaSqDNY77ApFzzfq4RtahenZmHs8zWjhiLnshAhCZbSH7MGqnpFSWrrLX5sgbYHnJDmE9NUqjS212KAW8"', true, 'Signature', 'new Signature("edsigtZ5u2yo1EfNLoxaPKafnmDZ6q1tjaP6deA7mX5dwx6GyPoN3Y3BfJv76jDcTAy9wsxkL1AQzFb4FvTWxLAtaXiS2dQg9gw")', { inactivate: true }),
  ...
  new item('or<nat, string>', 'right<nat>("")', true, 'Or<Nat, string>', 'Or.Left(new Nat(2))', { inactivate: true, name: 'or_nat_string' }),
...
`
  ...
  const gen_it = (item: item): string => {
    ...
    return `const v : Option<${item.ts_type}> = new Option<${item.ts_type}>(${item.ts_value});
    await type_option_${name}.deploy({ as: alice });
    await type_option_${name}.set_value(v, { as: alice });
