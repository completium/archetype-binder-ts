...
describe('Type set', async () => {
  ...
  it('enum_simple', async () => {
    const v : Array<type_set_enum_simple.enum_simple> = [new type_set_enum_simple.e_2()];
    ...
    assert(v.length == res.length && ((x : type_set_enum_simple.enum_simple, y : type_set_enum_simple.enum_simple) => {return x.toString() == y.toString()})(v[0], res[0]), "Invalid Value")
  });
