...
import { template_fa1_2 } from './contracts/bindings/template_fa1_2';
...
describe('[FA1.2] Contracts deployment', async () => {
  it('FA1.2 contract deployment should succeed', async () => {
    await template_fa1_2.deploy(alice.get_address(), total_supply, metadata_coin, { as: alice })
  });
});

describe('[FA1.2] Transfer', async () => {
  it('Check if balances are right', async () => {
    const alice_balance = await template_fa1_2.get_ledger_value(alice.get_address());
    assert(alice_balance?.tokens.equals(total_supply), "INVALID_BALANCE");

    const bob_balance = await template_fa1_2.get_ledger_value(bob.get_address());
    assert(bob_balance === undefined);
  })

  it('Transfer from <> caller without allowance should failed', async () => {
    await expect_to_fail(async () => {
      await template_fa1_2.transfer(alice.get_address(), bob.get_address(), new Nat(1), { as: bob });
    }, error_not_enough_allowance);
  })

  it('Transfer unkown from should failed', async () => {
    await expect_to_fail(async () => {
      await template_fa1_2.transfer(bob.get_address(), carl.get_address(), new Nat(1), { as: bob });
    }, error_ledger_not_found);
  })

  it('Transfer from = caller with insufficient balance should failed', async () => {
    await expect_to_fail(async () => {
      await template_fa1_2.transfer(alice.get_address(), bob.get_address(), total_supply.plus(new Nat(1)), { as: alice });
    }, template_fa1_2.errors.r1);
  })

  it('Transfer from = call with sufficient balance should succeed', async () => {
    const v = new Nat(total_supply.to_big_number().div(5));
    await template_fa1_2.transfer(alice.get_address(), bob.get_address(), v, { as: alice });
...
describe('[FA1.2] Allowance ', async () => {
  it('Approve known caller with 0 spender allowance should succeed', async () => {
    await template_fa1_2.approve( carl.get_address(), new Nat(0), {as: bob})
  })

  it('Approve unknown caller should succeed', async () => {
    await template_fa1_2.approve( carl.get_address(), new Nat(10), {as: user1})
  })

  it('Approve known caller with positive spender allowance should failed', async () => {
    await template_fa1_2.approve(carl.get_address(), new Nat(100), {as: bob})
  })
...
describe('[FA1.2] Allowance and Transfer ', async () => {
  ...
  it('Approve and transfer known caller with 0 spender allowance should succeed', async () => {
    const alice_balance_before = await template_fa1_2.get_ledger_value(alice.get_address());
    const bob_balance_before = await template_fa1_2.get_ledger_value(bob.get_address());
    const v = 100;

    await template_fa1_2.approve(carl.get_address(), new Nat(v), {as: alice})

    await template_fa1_2.transfer(alice.get_address(), bob.get_address(), new Nat(v), {as: carl});
...
describe('[FA1.2] Getters', async () => {
  ...
  it('Check getAllowance values', async () => {
    const value = await template_fa1_2.getAllowance(user1.get_address(), carl.get_address(), {as: alice} );
  ...
  it('Check getBalance values', async () => {
    ...
    const value = await template_fa1_2.getBalance(alice.get_address(), {as: alice});
    assert(alice_balance?.tokens.equals(value), "Invalid value for getBalance")
  })

  it('Check getTotalSupply value', async () => {
    const value = await template_fa1_2.getTotalSupply({as: alice});
