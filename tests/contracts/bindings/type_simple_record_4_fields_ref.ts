...
export class r_record implements att.ArchetypeType {
    ...
    static from_mich(input : att.Micheline): r_record {
        const p = (input as att.Mpair);
        return new r_record(att.mich_to_nat(p.args[0]), att.mich_to_string(p.args[1]), att.mich_to_bytes(p.args[2]), att.mich_to_bool(p.args[3]));
    }
}
