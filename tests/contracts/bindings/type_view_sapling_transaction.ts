...
export class Type_view_sapling_transaction {
    ...
    async view_get_value(i: att.Sapling_transaction, params: Partial<ex.Parameters>): Promise<att.Sapling_transaction | undefined> {
        if (this.address != undefined) {
            const mich = await ex.exec_view(this.get_address(), "get_value", view_get_value_arg_to_mich(i), params);
            return mich.value ? att.Sapling_transaction.from_mich(mich.value) : undefined;
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
