...
export class my_record implements att.ArchetypeType {
    constructor(public n: att.Nat, public v: att.Chest_key, public s: string) { }
    toString(): string {
    ...
    static from_mich(input: att.Micheline): my_record {
        return new my_record(att.Nat.from_mich((input as att.Mpair).args[0]), att.Chest_key.from_mich((input as att.Mpair).args[1]), att.mich_to_string((input as att.Mpair).args[2]));
    }
}
...
export class Type_record_chest_key {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_record_chest_key.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_record_chest_key = new Type_record_chest_key();
