...
export class Type_asset_key_1_iterable_big_map_tuple_nat_string_bytes_bool_custom {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_key_1_iterable_big_map_tuple_nat_string_bytes_bool_custom.arl", {}, params)).address;
        this.address = address;
    }
    async asset_put(i: [
        att.Nat,
    ...
    async get_my_asset_value(key: [
        ...
        if (this.address != undefined) {
            ...
            const raw_data = await ex.get_big_map_value(BigInt(att.Int.from_mich((storage as att.Mpair)?.args[0]).toString()), att.pair_to_mich([key[0].to_mich(), att.pair_to_mich([att.string_to_mich(key[1][0]), key[1][1].to_mich()]), att.bool_to_mich(key[2])]), my_asset_key_mich_type);
...
export const type_asset_key_1_iterable_big_map_tuple_nat_string_bytes_bool_custom = new Type_asset_key_1_iterable_big_map_tuple_nat_string_bytes_bool_custom();
