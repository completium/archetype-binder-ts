...
export class Type_tuple_enum_param {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_tuple_enum_param.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_res(): Promise<[
        ...
        if (this.address != undefined) {
            ...
            return (p => {
                return [att.Nat.from_mich((p as att.Mpair).args[0]), mich_to_e_enum((p as att.Mpair).args[1]), att.mich_to_string((p as att.Mpair).args[2])];
            })(storage);
        }
        throw new Error("Contract not initialised");
    }
...
export const type_tuple_enum_param = new Type_tuple_enum_param();
