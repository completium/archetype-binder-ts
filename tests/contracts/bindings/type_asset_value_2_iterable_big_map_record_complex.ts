...
export class Type_asset_value_2_iterable_big_map_record_complex {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_2_iterable_big_map_record_complex.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset_value(key: att.Nat): Promise<r_record | undefined> {
        if (this.address != undefined) {
            ...
            const data = raw_data ? att.pair_to_mich((raw_data as att.Mpair).args.slice(1, 11)) : undefined;
            if (data != undefined) {
...
export const type_asset_value_2_iterable_big_map_record_complex = new Type_asset_value_2_iterable_big_map_record_complex();
