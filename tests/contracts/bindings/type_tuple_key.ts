...
export const storage_type: att.MichelineType = { prim: "pair", args: [{ prim: "nat", annots: [] }, { prim: "pair", args: [{ prim: "key", annots: [] }, { prim: "string", annots: [] }], annots: [] }], annots: [] };
const set_value_arg_to_mich = (i: att.Key): att.Micheline => {
    return i.to_mich();
}
export class Type_tuple_key {
    ...
    async get_res(): Promise<[
        ...
        if (this.address != undefined) {
            ...
            return (p => {
                ...
                return [att.mich_to_nat(p0.args[0]), att.mich_to_key(p0.args[1]), att.mich_to_string(p1.args[0])];
            })(storage);
        }
        throw new Error("Contract not initialised");
    }
...
export const type_tuple_key = new Type_tuple_key();
