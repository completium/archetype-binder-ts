...
export class Type_getter_set_nat {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_getter_set_nat.arl", {}, params);
        this.address = address;
    ...
    async get_value(i: Array<att.Nat>, params: Partial<ex.Parameters>): Promise<Array<att.Nat>> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<Array<att.Nat>>(this.get_value_callback_address, x => { });
            }
        }
        throw new Error("Contract not initialised");
    }
