...
export const deploy_get_value_callback = async (params: Partial<ex.Parameters>): Promise<att.DeployResult> => {
    return await ex.deploy_callback("get_value", att.prim_annot_to_mich_type("mutez", []), params);
};
    ...
    async get_value(i: att.Tez, params: Partial<ex.Parameters>): Promise<att.Tez> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<att.Tez>(this.get_value_callback_address, x => { return att.Tez.from_mich(x); });
            }
        }
        throw new Error("Contract not initialised");
    }
