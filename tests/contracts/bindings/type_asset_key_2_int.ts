...
export class Type_asset_key_2_int {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_key_2_int.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset(): Promise<my_asset_container> {
        if (this.address != undefined) {
            ...
            for (let e of storage.entries()) {
                res.push([(x => { return new my_asset_key((x => { return new att.Int(x); })(x[0]), (x => { return new att.Nat(x); })(x[1])); })(e[0]), (x => { return x; })(e[1])]);
            }
            return res;
        }
        throw new Error("Contract not initialised");
