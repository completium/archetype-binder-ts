...
export const deploy_get_value_callback = async (): Promise<string> => {
    return await ex.deploy_callback("get_value", att.prim_annot_to_mich_type("bool", []));
};
export class Type_getter_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_getter_bool.arl", {}, params);
        this.address = address;
    ...
    async get_value(i: boolean, params: Partial<ex.Parameters>): Promise<boolean> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<boolean>(this.get_value_callback_address, x => { return x; });
            }
        }
        throw new Error("Contract not initialised");
    }
