...
export class my_record implements att.ArchetypeType {
    constructor(public n: att.Nat, public v: att.Signature, public s: string) { }
    toString(): string {
...
export const mich_to_my_record = (v: att.Micheline, collapsed: boolean = false): my_record => {
    ...
    return new my_record(att.mich_to_nat(fields[0]), att.mich_to_signature(fields[1]), att.mich_to_string(fields[2]));
};
const set_value_arg_to_mich = (i: my_record): att.Micheline => {
    return i.to_mich();
}
export class Type_record_signature {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_record_signature.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res(): Promise<my_record> {
        if (this.address != undefined) {
            ...
            return new my_record((x => { return new att.Nat(x); })(storage.n), (x => { return new att.Signature(x); })(storage.v), (x => { return x; })(storage.s));
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
}
export const type_record_signature = new Type_record_signature();
