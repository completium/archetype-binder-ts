...
export const my_asset_container_mich_type: att.MichelineType = att.pair_array_to_mich_type([
    att.pair_annot_to_mich_type("big_map", att.prim_annot_to_mich_type("int", []), att.pair_array_to_mich_type([
        att.prim_annot_to_mich_type("nat", ["%index"]),
        att.prim_annot_to_mich_type("string", ["%value"])
    ], ["%values"]), ["%values"]),
    att.pair_annot_to_mich_type("big_map", att.prim_annot_to_mich_type("nat", []), att.prim_annot_to_mich_type("int", []), ["%keys"]),
    att.prim_annot_to_mich_type("nat", ["%size"])
], []);
const asset_put_arg_to_mich = (i: att.Duration): att.Micheline => {
    return i.to_mich();
}
export class Type_asset_key_1_iterable_big_map_duration {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_key_1_iterable_big_map_duration.arl", {}, params)).address;
        this.address = address;
    }
    async asset_put(i: att.Duration, params: Partial<ex.Parameters>): Promise<att.CallResult> {
    ...
    async get_my_asset_value(key: att.Duration): Promise<string | undefined> {
        if (this.address != undefined) {
    ...
    async has_my_asset_value(key: att.Duration): Promise<boolean> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            const raw_data = await ex.get_big_map_value(BigInt(att.Int.from_mich((storage as att.Mpair)?.args[0]).toString()), key.to_mich(), my_asset_key_mich_type);
            const data = raw_data ? raw_data?.args[1] : undefined;
...
export const type_asset_key_1_iterable_big_map_duration = new Type_asset_key_1_iterable_big_map_duration();
