...
export class my_record implements att.ArchetypeType {
    ...
    static from_mich(input: att.Micheline): my_record {
        return new my_record(att.Nat.from_mich((input as att.Mpair).args[0]), att.Bls12_381_fr.from_mich((input as att.Mpair).args[1]), att.mich_to_string((input as att.Mpair).args[2]));
    }
}
...
export class Type_record_bls12_381_fr {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_record_bls12_381_fr.arl", {}, params)).address;
        this.address = address;
    }
