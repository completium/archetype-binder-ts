...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("big_map", att.prim_annot_to_mich_type("key", []), att.prim_annot_to_mich_type("unit", []), []);
const asset_put_arg_to_mich = (i: att.Key): att.Micheline => {
    return i.to_mich();
}
export class Type_asset_only_key_1_big_map_key {
    ...
    async get_my_asset_value(key: att.Key): Promise<my_asset_value | undefined> {
        if (this.address != undefined) {
...
export const type_asset_only_key_1_big_map_key = new Type_asset_only_key_1_big_map_key();
