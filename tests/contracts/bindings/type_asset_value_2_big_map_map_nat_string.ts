...
export class my_asset_value implements att.ArchetypeType {
    ...
    to_mich(): att.Micheline {
        return att.list_to_mich(this.v, x => {
            const x_key = x[0];
            const x_value = x[1];
    ...
    equals(v: my_asset_value): boolean {
        return JSON.stringify(this.v) == JSON.stringify(v.v);
    }
    static from_mich(input: att.Micheline): my_asset_value {
        return new my_asset_value(att.mich_to_map(input, (x, y) => [att.mich_to_nat(x), att.mich_to_string(y)]));
    }
}
...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("big_map", att.prim_annot_to_mich_type("nat", []), att.pair_annot_to_mich_type("map", att.prim_annot_to_mich_type("nat", []), att.prim_annot_to_mich_type("string", []), []), []);
...
export class Type_asset_value_2_big_map_map_nat_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_2_big_map_map_nat_string.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_value_2_big_map_map_nat_string = new Type_asset_value_2_big_map_map_nat_string();
