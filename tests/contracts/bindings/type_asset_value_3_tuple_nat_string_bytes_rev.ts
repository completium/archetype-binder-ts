...
export class Type_asset_value_3_tuple_nat_string_bytes_rev {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_asset_value_3_tuple_nat_string_bytes_rev.arl", {}, params);
        this.address = address;
    }
    async asset_put(i: [
        [
    ...
    async get_my_asset(): Promise<my_asset_container> {
        if (this.address != undefined) {
            ...
            for (let e of storage.entries()) {
                res.push([(x => { return new att.Nat(x); })(e[0]), (x => { return new my_asset_value((x => { return x; })(x.s), (x => { return [[(x => { return new att.Nat(x); })(x[Object.keys(x)[0]]), (x => { return x; })(x[Object.keys(x)[1]])], (x => { return new att.Bytes(x); })(x[Object.keys(x)[2]])]; })(x.v)); })(e[1])]);
            }
            return res;
        }
        throw new Error("Contract not initialised");
