...
export const storage_type: att.MichelineType = { prim: "big_map", args: [{ prim: "pair", args: [{ prim: "nat", annots: ["%f_a"] }, { prim: "pair", args: [{ prim: "pair", args: [{ prim: "string", annots: ["%f_b"] }, { prim: "bytes", annots: ["%f_c"] }], annots: [] }, { prim: "bool", annots: ["%f_d"] }], annots: [] }], annots: [] }, { prim: "nat", annots: [] }], annots: [] };
...
export class Type_big_map_key_record_4_fields_custom {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_big_map_key_record_4_fields_custom.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_res_value(key: r_record): Promise<att.Nat | undefined> {
        if (this.address != undefined) {
            ...
            const data = await ex.get_big_map_value(BigInt(storage), key.to_mich(), att.prim_annot_to_mich_type("record", []), att.prim_annot_to_mich_type("nat", [])), collapsed = true;
            if (data != undefined) {
...
export const type_big_map_key_record_4_fields_custom = new Type_big_map_key_record_4_fields_custom();
