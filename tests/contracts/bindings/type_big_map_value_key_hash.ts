...
export class Type_big_map_value_key_hash {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_big_map_value_key_hash.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res_value(key: att.Nat): Promise<att.Key_hash | undefined> {
        if (this.address != undefined) {
            ...
            const data = await ex.get_big_map_value(BigInt(storage), key.to_mich(), att.prim_annot_to_mich_type("nat", []), att.prim_annot_to_mich_type("key_hash", [])), collapsed = true;
            if (data != undefined) {
                return new att.Key_hash(data);
            }
            else {
                return undefined;
            }
