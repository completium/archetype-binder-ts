...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("big_map", att.prim_annot_to_mich_type("key_hash", []), att.prim_annot_to_mich_type("unit", []), []);
const asset_put_arg_to_mich = (i: att.Key_hash): att.Micheline => {
    return i.to_mich();
}
export class Type_asset_only_key_1_big_map_key_hash {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_only_key_1_big_map_key_hash.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset_value(key: att.Key_hash): Promise<my_asset_value | undefined> {
        if (this.address != undefined) {
...
export const type_asset_only_key_1_big_map_key_hash = new Type_asset_only_key_1_big_map_key_hash();
