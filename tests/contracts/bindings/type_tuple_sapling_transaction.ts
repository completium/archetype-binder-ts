...
export class Type_tuple_sapling_transaction {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_tuple_sapling_transaction.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res(): Promise<[
        ...
        if (this.address != undefined) {
            ...
            return [(x => { return new att.Nat(x); })(storage[Object.keys(storage)[0]]), (x => { return new att.Sapling_transaction(x); })(storage[Object.keys(storage)[1]]), (x => { return x; })(storage[Object.keys(storage)[2]])];
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
