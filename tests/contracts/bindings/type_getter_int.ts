...
export const storage_type: att.MichelineType = { prim: "int", annots: [] };
const get_value_arg_to_mich = (i: att.Int): att.Micheline => {
    return i.to_mich();
}
...
export class Type_getter_int {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_getter_int.arl", {}, params)).address;
        this.address = address;
        this.get_value_callback_address = (await deploy_get_value_callback()).address;
    }
    async get_value(i: att.Int, params: Partial<ex.Parameters>): Promise<att.Int> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<att.Int>(this.get_value_callback_address, x => { return att.mich_to_int(x); });
            }
        }
        throw new Error("Contract not initialised");
    }
...
export const type_getter_int = new Type_getter_int();
