...
export class my_asset_value implements att.ArchetypeType {
    constructor(public s: string, public v: att.Rational) { }
    toString(): string {
    ...
    static from_mich(input: att.Micheline): my_asset_value {
        return new my_asset_value(att.mich_to_string((input as att.Mpair).args[0]), att.Rational.from_mich(att.pair_to_mich((input as att.Mpair as att.Mpair).args.slice(1, 3))));
    }
}
...
export class Type_asset_value_3_iterable_big_map_rational {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_3_iterable_big_map_rational.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_value_3_iterable_big_map_rational = new Type_asset_value_3_iterable_big_map_rational();
