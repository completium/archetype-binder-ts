...
export class Type_getter_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_getter_string.arl", {}, params);
        this.address = address;
    ...
    async get_value(i: string, params: Partial<ex.Parameters>): Promise<string> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<string>(this.get_value_callback_address, x => { });
            }
        }
        throw new Error("Contract not initialised");
    }
