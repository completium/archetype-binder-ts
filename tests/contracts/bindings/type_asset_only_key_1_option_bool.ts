...
export const my_asset_container_mich_type: att.MichelineType = att.set_annot_to_mich_type(att.option_annot_to_mich_type(att.prim_annot_to_mich_type("bool", []), []), []);
const asset_put_arg_to_mich = (i: att.Option<boolean>): att.Micheline => {
    return i.to_mich((x => { return att.bool_to_mich(x); }));
}
export class Type_asset_only_key_1_option_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_only_key_1_option_bool.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_only_key_1_option_bool = new Type_asset_only_key_1_option_bool();
