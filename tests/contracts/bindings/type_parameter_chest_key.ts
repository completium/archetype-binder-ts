...
export class Type_asset_parameter_chest_key {
    ...
    async deploy(res: att.Chest_key, params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_asset_parameter_chest_key.arl", {
            res: res.to_mich()
        }, params);
        this.address = address;
    }
...
export const type_asset_parameter_chest_key = new Type_asset_parameter_chest_key();
