...
export const storage_type: att.MichelineType = { prim: "pair", args: [{ prim: "nat", annots: ["%n"] }, { prim: "pair", args: [{ prim: "pair", args: [{ prim: "nat", annots: [] }, { prim: "pair", args: [{ prim: "string", annots: [] }, { prim: "bytes", annots: [] }], annots: [] }], annots: ["%v"] }, { prim: "string", annots: ["%s"] }], annots: [] }], annots: [] };
export class my_record implements att.ArchetypeType {
    ...
    to_mich(): att.Micheline {
        return att.pair_to_mich([this.n.to_mich(), att.pair_to_mich([att.pair_to_mich([this.v[0].to_mich(), att.string_to_mich(this.v[1]), this.v[2].to_mich()]), att.string_to_mich(this.s)])]);
    }
...
export class Type_record_tuple_nat_string_bytes {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_record_tuple_nat_string_bytes.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_record_tuple_nat_string_bytes = new Type_record_tuple_nat_string_bytes();
