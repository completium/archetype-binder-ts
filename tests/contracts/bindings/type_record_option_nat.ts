...
export class my_record implements att.ArchetypeType {
    ...
    to_mich(): att.Micheline {
        return att.pair_to_mich([this.n.to_mich(), this.v.to_mich((x => { return x.to_mich(); })), att.string_to_mich(this.s)]);
    }
    equals(v: my_record): boolean {
        return att.micheline_equals(this.to_mich(), v.to_mich());
    }
    static from_mich(input: att.Micheline): my_record {
        return new my_record(att.Nat.from_mich((input as att.Mpair).args[0]), att.Option.from_mich((input as att.Mpair).args[1], x => { return att.Nat.from_mich(x); }), att.mich_to_string((input as att.Mpair).args[2]));
    }
}
...
export class Type_record_option_nat {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_record_option_nat.arl", {}, params)).address;
        this.address = address;
    }
