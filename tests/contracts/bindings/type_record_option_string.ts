...
export class my_record implements att.ArchetypeType {
    ...
    static from_mich(input: att.Micheline): my_record {
        return new my_record(att.mich_to_nat((input as att.Mpair).args[0]), att.mich_to_option((input as att.Mpair).args[1], x => { return att.mich_to_string(x); }), att.mich_to_string((input as att.Mpair).args[2]));
    }
}
