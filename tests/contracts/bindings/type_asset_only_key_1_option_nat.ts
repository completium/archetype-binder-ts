...
export const my_asset_container_mich_type: att.MichelineType = att.set_annot_to_mich_type(att.option_annot_to_mich_type(att.prim_annot_to_mich_type("nat", []), []), []);
const asset_put_arg_to_mich = (i: att.Option<att.Nat>): att.Micheline => {
    return i.to_mich((x => { return x.to_mich(); }));
}
export class Type_asset_only_key_1_option_nat {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_only_key_1_option_nat.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_only_key_1_option_nat = new Type_asset_only_key_1_option_nat();
