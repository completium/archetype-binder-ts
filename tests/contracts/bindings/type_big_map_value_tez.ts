...
export class Type_big_map_value_tez {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_big_map_value_tez.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res_value(key: att.Nat): Promise<att.Tez | undefined> {
        if (this.address != undefined) {
            ...
            const data = await ex.get_big_map_value(BigInt(storage), key.to_mich(), att.prim_annot_to_mich_type("nat", []), att.prim_annot_to_mich_type("currency", [])), collapsed = true;
            if (data != undefined) {
                return new att.Tez(data, "mutez");
            }
            else {
                return undefined;
            }
