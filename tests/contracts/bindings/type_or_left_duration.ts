...
export class Type_or_left_duration {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_or_left_duration.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res(): Promise<att.Or<att.Duration, att.Nat>> {
        if (this.address != undefined) {
            ...
            return (x => {
                ...
                const value = is_left ? (x => { return new att.Duration(x["0"]); })(x["0"]) : (x => { return new att.Nat(x["1"]); })(x["1"]);
