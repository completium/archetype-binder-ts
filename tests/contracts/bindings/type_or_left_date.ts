...
export const storage_type: att.MichelineType = { prim: "or", args: [{ prim: "timestamp", annots: [] }, { prim: "nat", annots: [] }], annots: [] };
...
export class Type_or_left_date {
    ...
    async get_res(): Promise<att.Or<Date, att.Nat>> {
        if (this.address != undefined) {
            ...
            return att.mich_to_or(storage, x => { return att.mich_to_date(x); }, x => { return att.mich_to_nat(x); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
