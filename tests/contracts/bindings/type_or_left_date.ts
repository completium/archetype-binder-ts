...
export class Type_or_left_date {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const res = await ex.deploy("./tests/contracts/type_or_left_date.arl", {}, params);
    ...
    async get_res(): Promise<att.Or<Date, Date>> {
        if (this.address != undefined) {
            const storage = await ex.get_storage(this.address);
            return (x => {
                const is_left = x["0"] !== undefined;
                const value = is_left ? (x => { return new Date(x); })(x["0"]) : (x => { return new Date(x); })(x["1"]);
                return new att.Or<Date, Date>(value, is_left);
            })(storage);
        }
        throw new Error("Contract not initialised");
    }
