...
export class Type_view_enum_param {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_view_enum_param.arl", {}, params)).address;
        this.address = address;
    }
    async view_get_value(i: e_enum, params: Partial<ex.Parameters>): Promise<e_enum | undefined> {
        if (this.address != undefined) {
            const mich = await ex.exec_view(this.get_address(), "get_value", view_get_value_arg_to_mich(i), params);
            return mich.value ? mich_to_e_enum(mich.value) : undefined;
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
}
export const type_view_enum_param = new Type_view_enum_param();
