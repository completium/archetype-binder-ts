...
export const storage_type: att.MichelineType = { prim: "list", args: [{ prim: "string", annots: [] }], annots: [] };
const set_value_arg_to_mich = (i: Array<string>): att.Micheline => {
    return att.list_to_mich(i, x => {
        return att.string_to_mich(x);
    });
}
export class Type_simple_list_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_simple_list_string.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_simple_list_string = new Type_simple_list_string();
