...
export class Type_getter_record_2_fields {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_getter_record_2_fields.arl", {}, params);
        this.address = address;
    ...
    async get_value(i: r_record, params: Partial<ex.Parameters>): Promise<r_record> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<r_record>(this.get_value_callback_address, x => { });
            }
        }
        throw new Error("Contract not initialised");
    }
