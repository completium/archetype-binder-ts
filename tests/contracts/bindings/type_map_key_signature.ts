...
export const storage_type: att.MichelineType = { prim: "map", args: [{ prim: "signature", annots: [] }, { prim: "nat", annots: [] }], annots: [] };
const set_value_arg_to_mich = (i: att.Signature): att.Micheline => {
    return i.to_mich();
}
export class Type_map_key_signature {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_map_key_signature.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_map_key_signature = new Type_map_key_signature();
