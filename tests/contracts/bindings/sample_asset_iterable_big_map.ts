...
export class Sample_asset_iterable_big_map {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/sample_asset_iterable_big_map.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset_value(key: string): Promise<att.Nat | undefined> {
        if (this.address != undefined) {
            ...
            const data = (await ex.get_big_map_value(BigInt(att.Int.from_mich((storage as att.Mpair)?.args[0]).toString()), att.string_to_mich(key), my_asset_key_mich_type) as att.Mpair)?.args[1];
            if (data != undefined) {
...
export const sample_asset_iterable_big_map = new Sample_asset_iterable_big_map();
