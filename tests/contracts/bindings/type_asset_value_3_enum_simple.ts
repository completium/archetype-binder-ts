...
export class Type_asset_value_3_enum_simple {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_3_enum_simple.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset(): Promise<my_asset_container> {
        if (this.address != undefined) {
            ...
            for (let e of storage.entries()) {
                res.push([(x => { return new att.Nat(x); })(e[0]), (x => { return new my_asset_value((x => { return x; })(x.s), (x => { if (x == "2" || (x.toNumber ? x.toNumber() == 2 : false)) {
                        return new e_3();
                    }
...
export const type_asset_value_3_enum_simple = new Type_asset_value_3_enum_simple();
