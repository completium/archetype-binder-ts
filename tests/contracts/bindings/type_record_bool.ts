...
export class my_record implements att.ArchetypeType {
    ...
    to_mich(): att.Micheline {
        return att.pair_to_mich([this.n.to_mich(), att.bool_to_mich(this.v), att.string_to_mich(this.s)]);
    }
    equals(v: my_record): boolean {
        return att.micheline_equals(this.to_mich(), v.to_mich());
    }
    static from_mich(input: att.Micheline): my_record {
        return new my_record(att.Nat.from_mich((input as att.Mpair).args[0]), att.mich_to_bool((input as att.Mpair).args[1]), att.mich_to_string((input as att.Mpair).args[2]));
    }
}
