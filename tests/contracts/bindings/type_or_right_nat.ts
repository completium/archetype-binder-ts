...
export class Type_or_right_nat {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_or_right_nat.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res(): Promise<att.Or<att.Nat, att.Nat>> {
        if (this.address != undefined) {
            ...
            return (x => {
                ...
                const value = is_left ? (x => { return new att.Nat(x["0"]); })(x["0"]) : (x => { return new att.Nat(x["1"]); })(x["1"]);
