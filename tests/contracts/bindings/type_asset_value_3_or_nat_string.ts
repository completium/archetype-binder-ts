...
export class my_asset_value implements att.ArchetypeType {
    constructor(public s: string, public v: att.Or<att.Nat, att.Nat>) { }
    toString(): string {
        return JSON.stringify(this, null, 2);
    }
    to_mich(): att.Micheline {
        return att.pair_to_mich([att.string_to_mich(this.s), this.v.to_mich((x => { return x.to_mich(); }), (x => { return x.to_mich(); }))]);
    }
...
export class Type_asset_value_3_or_nat_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const res = await ex.deploy("./tests/contracts/type_asset_value_3_or_nat_string.arl", {}, params);
