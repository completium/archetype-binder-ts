...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("big_map", att.prim_annot_to_mich_type("unit", []), att.prim_annot_to_mich_type("string", []), []);
const asset_put_arg_to_mich = (i: att.Unit): att.Micheline => {
    return att.unit_to_mich();
}
export class Type_asset_key_1_big_map_unit {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_key_1_big_map_unit.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset_value(key: my_asset_key): Promise<my_asset_value | undefined> {
        if (this.address != undefined) {
            ...
            const data = await ex.get_big_map_value(BigInt(att.mich_to_int(storage).toString()), att.unit_to_mich(), my_asset_key_mich_type);
            if (data != undefined) {
...
export const type_asset_key_1_big_map_unit = new Type_asset_key_1_big_map_unit();
