...
export class my_record implements att.ArchetypeType {
    constructor(public n: att.Nat, public v: Array<att.Nat>, public s: string) { }
    toString(): string {
...
export const mich_to_my_record = (v: att.Micheline, collapsed: boolean = false): my_record => {
    ...
    return new my_record(att.mich_to_nat(fields[0]), att.mich_to_list(fields[1], x => { return att.mich_to_nat(x); }), att.mich_to_string(fields[2]));
};
const set_value_arg_to_mich = (i: my_record): att.Micheline => {
    return i.to_mich();
}
export class Type_record_list_nat {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_record_list_nat.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res(): Promise<my_record> {
        if (this.address != undefined) {
            ...
            return new my_record((x => { return new att.Nat(x); })(storage.n), (x => { const res: Array<att.Nat> = []; for (let i = 0; i < x.length; i++) {
...
export const type_record_list_nat = new Type_record_list_nat();
