...
export class my_asset_value implements att.ArchetypeType {
    constructor(public v: att.Rational) { }
    toString(): string {
        return JSON.stringify(this, null, 2);
    }
    to_mich(): att.Micheline {
        return att.pair_to_mich([this.v.to_mich(), this.v.to_mich()]);
    }
    equals(v: my_asset_value): boolean {
        return this.v.equals(v.v);
    }
    static from_mich(input: att.Micheline): my_asset_value {
        return new my_asset_value(att.mich_to_rational(input));
    }
}
...
export class Type_asset_value_2_big_map_rational {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_2_big_map_rational.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_value_2_big_map_rational = new Type_asset_value_2_big_map_rational();
