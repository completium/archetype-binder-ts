...
export class my_asset_key implements att.ArchetypeType {
    ...
    static from_mich(input: att.Micheline): my_asset_key {
        return new my_asset_key(att.Or.from_mich((input as att.Mpair).args[0], x => { return att.Nat.from_mich(x); }, x => { return att.mich_to_string(x); }), att.Nat.from_mich((input as att.Mpair).args[1]));
    }
}
...
export const my_asset_container_mich_type: att.MichelineType = att.pair_array_to_mich_type([
    att.pair_annot_to_mich_type("big_map", att.pair_array_to_mich_type([
        att.or_to_mich_type(att.prim_annot_to_mich_type("nat", []), att.prim_annot_to_mich_type("string", []), []),
...
export class Type_asset_key_2_iterable_big_map_or_nat_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_key_2_iterable_big_map_or_nat_string.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_key_2_iterable_big_map_or_nat_string = new Type_asset_key_2_iterable_big_map_or_nat_string();
