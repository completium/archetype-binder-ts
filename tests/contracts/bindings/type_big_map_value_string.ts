...
export const storage_type: att.MichelineType = { prim: "big_map", args: [{ prim: "nat", annots: [] }, { prim: "string", annots: [] }], annots: [] };
...
export class Type_big_map_value_string {
    ...
    async get_res_value(key: att.Nat): Promise<string | undefined> {
        if (this.address != undefined) {
            ...
            const data = await ex.get_big_map_value(BigInt(storage), key.to_mich(), att.prim_annot_to_mich_type("nat", []), att.prim_annot_to_mich_type("string", [])), collapsed = true;
            if (data != undefined) {
