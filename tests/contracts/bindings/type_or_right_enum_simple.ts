...
export class Type_or_right_enum_simple {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_or_right_enum_simple.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res(): Promise<att.Or<att.Nat, enum_simple>> {
        if (this.address != undefined) {
            const storage = await ex.get_storage(this.address);
            return (x => {
                const is_left = x["0"] !== undefined;
                const value = is_left ? (x => { return new att.Nat(x["0"]); })(x["0"]) : (x => { if (x["1"].e_3 !== undefined) {
                    return new e_3();
                }
                else if (x["1"].e_2 !== undefined) {
                    return new e_2();
                }
                else
                    return new e_1(); })(x["1"]);
                return new att.Or<att.Nat, enum_simple>(value, is_left);
            })(storage);
        }
        throw new Error("Contract not initialised");
    }
