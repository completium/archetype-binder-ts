...
export class Type_asset_parameter_chest {
    ...
    async deploy(res: att.Chest, params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_asset_parameter_chest.arl", {
            res: res.to_mich()
        }, params);
        this.address = address;
    }
...
export const type_asset_parameter_chest = new Type_asset_parameter_chest();
