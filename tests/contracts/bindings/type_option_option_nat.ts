...
export const storage_type: att.MichelineType = { prim: "option", args: [{ prim: "option", args: [{ prim: "nat", annots: [] }], annots: [] }], annots: [] };
...
export class Type_option_option_nat {
    ...
    async get_res(): Promise<att.Option<att.Option<att.Nat>>> {
        if (this.address != undefined) {
            ...
            return att.mich_to_option(storage, x => { return att.mich_to_option(x, x => { return att.mich_to_nat(x); }); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
