...
export class Type_asset_only_key_1_big_map_tuple_nat_string_bytes_rev {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_only_key_1_big_map_tuple_nat_string_bytes_rev.arl", {}, params)).address;
        this.address = address;
    }
    async asset_put(i: [
        [
    ...
    async get_my_asset_value(key: [
        ...
        if (this.address != undefined) {
            ...
            const data = await ex.get_big_map_value(BigInt(att.Int.from_mich(storage).toString()), att.pair_to_mich([att.pair_to_mich([key[0][0].to_mich(), att.string_to_mich(key[0][1])]), key[1].to_mich()]), my_asset_key_mich_type);
            if (data != undefined) {
...
export const type_asset_only_key_1_big_map_tuple_nat_string_bytes_rev = new Type_asset_only_key_1_big_map_tuple_nat_string_bytes_rev();
