...
export class Type_or_left_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const res = await ex.deploy("./tests/contracts/type_or_left_string.arl", {}, params);
    ...
    async get_res(): Promise<att.Or<string, string>> {
        if (this.address != undefined) {
            const storage = await ex.get_storage(this.address);
            return (x => {
                const is_left = x["0"] !== undefined;
                const value = is_left ? (x => { return x; })(x["0"]) : (x => { return x; })(x["1"]);
                return new att.Or<string, string>(value, is_left);
            })(storage);
        }
        throw new Error("Contract not initialised");
    }
