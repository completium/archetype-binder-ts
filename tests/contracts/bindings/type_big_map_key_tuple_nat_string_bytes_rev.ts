...
export const storage_type: att.MichelineType = { prim: "big_map", args: [{ prim: "pair", args: [{ prim: "pair", args: [{ prim: "nat", annots: [] }, { prim: "string", annots: [] }], annots: [] }, { prim: "bytes", annots: [] }], annots: [] }, { prim: "nat", annots: [] }], annots: [] };
...
export class Type_big_map_key_tuple_nat_string_bytes_rev {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_big_map_key_tuple_nat_string_bytes_rev.arl", {}, params)).address;
        this.address = address;
    }
    async set_value(i: [
        [
    ...
    async get_res_value(key: [
        ...
        if (this.address != undefined) {
            ...
            const data = await ex.get_big_map_value(BigInt(storage), att.pair_to_mich([att.pair_to_mich([key[0][0].to_mich(), att.string_to_mich(key[0][1])]), key[1].to_mich()]), att.pair_array_to_mich_type([
