...
export const deploy_get_value_callback = async (params: Partial<ex.Parameters>): Promise<att.DeployResult> => {
    return await ex.deploy_callback("get_value", att.prim_annot_to_mich_type("sapling_transaction(8)", []), params);
};
    ...
    async get_value(i: att.Sapling_transaction, params: Partial<ex.Parameters>): Promise<att.Sapling_transaction> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<att.Sapling_transaction>(this.get_value_callback_address, x => { return new att.Sapling_transaction(x); });
            }
        }
        throw new Error("Contract not initialised");
    }
