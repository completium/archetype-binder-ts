...
export class Type_getter_tuple_nat_string_bytes_bool_custom {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_getter_tuple_nat_string_bytes_bool_custom.arl", {}, params)).address;
        this.address = address;
        ...
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<[
                    ...
                    boolean
                ]>(this.get_value_callback_address, x => { return [(x => { return new att.Nat(x); })(x[Object.keys(x)[0]]), [(x => { return x; })(x[Object.keys(x)[1]]), (x => { return new att.Bytes(x); })(x[Object.keys(x)[2]])], (x => { return x.prim ? (x.prim == "True" ? true : false) : x; })(x[Object.keys(x)[3]])]; });
            }
        }
        throw new Error("Contract not initialised");
    }
...
export const type_getter_tuple_nat_string_bytes_bool_custom = new Type_getter_tuple_nat_string_bytes_bool_custom();
