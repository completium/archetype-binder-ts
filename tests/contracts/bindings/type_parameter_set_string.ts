...
export const storage_type: att.MichelineType = { prim: "set", args: [{ prim: "string", annots: [] }], annots: [] };
const asset_add_arg_to_mich = (i: Array<string>): att.Micheline => {
    return att.list_to_mich(i, x => {
        return att.string_to_mich(x);
    });
}
export class Type_parameter_set_string {
    ...
    async deploy(res: Array<string>, params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_parameter_set_string.arl", {
    ...
    async asset_add(i: Array<string>, params: Partial<ex.Parameters>): Promise<any> {
...
export const type_parameter_set_string = new Type_parameter_set_string();
