...
export class Type_list_chest_key {
    ...
    async set_value(i: Array<att.Chest_key>, params: Partial<ex.Parameters>): Promise<att.CallResult> {
    ...
    async get_res(): Promise<Array<att.Chest_key>> {
        if (this.address != undefined) {
            ...
            return att.mich_to_list(storage, x => { return att.Chest_key.from_mich(x); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
