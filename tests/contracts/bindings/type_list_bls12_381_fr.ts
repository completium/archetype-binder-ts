...
export class Type_list_bls12_381_fr {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_list_bls12_381_fr.arl", {}, params)).address;
        this.address = address;
    }
    async set_value(i: Array<att.Bls12_381_fr>, params: Partial<ex.Parameters>): Promise<att.CallResult> {
    ...
    async get_res(): Promise<Array<att.Bls12_381_fr>> {
        if (this.address != undefined) {
            ...
            return att.mich_to_list(storage, x => { return att.Bls12_381_fr.from_mich(x); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
