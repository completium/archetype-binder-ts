...
export class Type_or_right_enum_param {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_or_right_enum_param.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_res(): Promise<att.Or<att.Nat, e_enum>> {
        if (this.address != undefined) {
            ...
            return att.Or.from_mich(storage, x => { return att.Nat.from_mich(x); }, x => { return mich_to_e_enum(x); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
}
export const type_or_right_enum_param = new Type_or_right_enum_param();
