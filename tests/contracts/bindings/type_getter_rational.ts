...
export class Type_getter_rational {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_getter_rational.arl", {}, params);
        this.address = address;
    ...
    async get_value(i: att.Rational, params: Partial<ex.Parameters>): Promise<att.Rational> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<att.Rational>(this.get_value_callback_address, x => { return new att.Rational(x[Object.keys(x)[0]], x[Object.keys(x)[1]]); });
            }
        }
        throw new Error("Contract not initialised");
    }
