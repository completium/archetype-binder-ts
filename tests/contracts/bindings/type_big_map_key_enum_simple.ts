...
export class Type_big_map_key_enum_simple {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_big_map_key_enum_simple.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res_value(key: enum_simple): Promise<att.Nat | undefined> {
        if (this.address != undefined) {
            const storage = await ex.get_storage(this.address);
            const data = await ex.get_big_map_value(BigInt(storage), key.to_mich(), att.prim_annot_to_mich_type("enum", []), att.prim_annot_to_mich_type("nat", [])), collapsed = true;
            if (data != undefined) {
    ...
    async has_res_value(key: enum_simple): Promise<boolean> {
        if (this.address != undefined) {
            const storage = await ex.get_storage(this.address);
            const data = await ex.get_big_map_value(BigInt(storage), key.to_mich(), att.prim_annot_to_mich_type("enum", []), att.prim_annot_to_mich_type("nat", [])), collapsed = true;
            if (data != undefined) {
