...
export class my_asset_key implements att.ArchetypeType {
    constructor(public k: att.Tez, public n: att.Nat) { }
    toString(): string {
    ...
    static from_mich(input: att.Micheline): my_asset_key {
        return new my_asset_key(att.Tez.from_mich((input as att.Mpair).args[0]), att.Nat.from_mich((input as att.Mpair).args[1]));
    }
}
export const my_asset_key_mich_type: att.MichelineType = att.pair_array_to_mich_type([
    att.prim_annot_to_mich_type("mutez", ["%k"]),
...
export const my_asset_container_mich_type: att.MichelineType = att.pair_array_to_mich_type([
    att.pair_annot_to_mich_type("big_map", att.pair_array_to_mich_type([
        att.prim_annot_to_mich_type("mutez", []),
...
export class Type_asset_key_2_iterable_big_map_tez {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_key_2_iterable_big_map_tez.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_key_2_iterable_big_map_tez = new Type_asset_key_2_iterable_big_map_tez();
