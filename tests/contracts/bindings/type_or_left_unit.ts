...
export class Type_or_left_unit {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const res = await ex.deploy("./tests/contracts/type_or_left_unit.arl", {}, params);
    ...
    async get_res(): Promise<att.Or<att.Unit, att.Unit>> {
        if (this.address != undefined) {
            const storage = await ex.get_storage(this.address);
            return (x => {
                const is_left = x["0"] !== undefined;
                const value = is_left ? (x => { return new att.Unit(); })(x["0"]) : (x => { return new att.Unit(); })(x["1"]);
                return new att.Or<att.Unit, att.Unit>(value, is_left);
            })(storage);
        }
        throw new Error("Contract not initialised");
    }
