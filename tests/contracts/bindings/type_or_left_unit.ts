...
export class Type_or_left_unit {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_or_left_unit.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res(): Promise<att.Or<att.Unit, att.Nat>> {
        if (this.address != undefined) {
            ...
            return (x => {
                ...
                const value = is_left ? (x => { return new att.Unit(x["0"]); })(x["0"]) : (x => { return new att.Nat(x["1"]); })(x["1"]);
                return new att.Or<att.Unit, att.Nat>(value, is_left);
            })(storage);
        }
        throw new Error("Contract not initialised");
    }
