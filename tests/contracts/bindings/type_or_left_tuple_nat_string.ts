...
export class Type_or_left_tuple_nat_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_or_left_tuple_nat_string.arl", {}, params);
        this.address = address;
    }
    async set_value(i: [
        att.Nat,
    ...
    async get_res(): Promise<att.Or<[
        ...
        if (this.address != undefined) {
            ...
            return (x => {
                ...
                const value = is_left ? (x => { return [(x => { return new att.Nat(x); })(x["0"][Object.keys(x["0"])[0]]), (x => { return x; })(x["0"][Object.keys(x["0"])[1]])]; })(x["0"]) : (x => { return new att.Nat(x["1"]); })(x["1"]);
                return new att.Or<[
                    att.Nat,
                    string
