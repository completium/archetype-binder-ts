...
export const storage_type: att.MichelineType = { prim: "big_map", args: [{ prim: "key_hash", annots: [] }, { prim: "nat", annots: [] }], annots: [] };
...
export class Type_big_map_key_key_hash {
    ...
    async get_res_value(key: att.Key_hash): Promise<att.Nat | undefined> {
        if (this.address != undefined) {
            ...
            const data = await ex.get_big_map_value(BigInt(storage), key.to_mich(), att.prim_annot_to_mich_type("key_hash", []), att.prim_annot_to_mich_type("nat", [])), collapsed = true;
            if (data != undefined) {
