...
export class Type_asset_value_2_list_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_2_list_bool.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset(): Promise<my_asset_container> {
        if (this.address != undefined) {
            ...
            return att.mich_to_map(storage, (x, y) => [att.Nat.from_mich(x), att.mich_to_list(y, x => { return att.mich_to_bool(x); })]);
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
}
export const type_asset_value_2_list_bool = new Type_asset_value_2_list_bool();
