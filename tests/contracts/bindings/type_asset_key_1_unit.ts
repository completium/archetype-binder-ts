...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("map", att.prim_annot_to_mich_type("unit", []), att.prim_annot_to_mich_type("string", []), []);
...
export class Type_asset_key_1_unit {
    ...
    async get_my_asset(): Promise<my_asset_container> {
        if (this.address != undefined) {
            ...
            return att.mich_to_map(storage, (x, y) => [new att.Unit(), att.mich_to_string(y)]);
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
