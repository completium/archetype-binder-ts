...
export class my_asset_value implements att.ArchetypeType {
    constructor(public v: Array<string>) { }
    toString(): string {
        return JSON.stringify(this, null, 2);
    }
    to_mich(): att.Micheline {
    ...
    static from_mich(input: att.Micheline): my_asset_value {
        return new my_asset_value(att.mich_to_list(input, x => { return att.mich_to_string(x); }));
    }
}
...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("big_map", att.prim_annot_to_mich_type("nat", []), att.list_annot_to_mich_type(att.prim_annot_to_mich_type("string", []), []), []);
const asset_put_arg_to_mich = (i: Array<string>): att.Micheline => {
    return att.list_to_mich(i, x => {
        return att.string_to_mich(x);
    });
}
export class Type_asset_value_2_big_map_list_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_2_big_map_list_string.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_value_2_big_map_list_string = new Type_asset_value_2_big_map_list_string();
