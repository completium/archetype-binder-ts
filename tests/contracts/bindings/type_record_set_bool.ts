...
export const mich_to_my_record = (v: att.Micheline, collapsed: boolean = false): my_record => {
    ...
    return new my_record(att.mich_to_nat(fields[0]), att.mich_to_list(fields[1], x => { return att.mich_to_bool(x); }), att.mich_to_string(fields[2]));
};
...
export class Type_record_set_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_record_set_bool.arl", {}, params);
        this.address = address;
    }
