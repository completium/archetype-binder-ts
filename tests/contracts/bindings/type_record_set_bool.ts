...
export class my_record implements att.ArchetypeType {
    ...
    static from_mich(input: att.Micheline): my_record {
        return new my_record(att.Nat.from_mich((input as att.Mpair).args[0]), att.mich_to_list((input as att.Mpair).args[1], x => { return att.mich_to_bool(x); }), att.mich_to_string((input as att.Mpair).args[2]));
    }
}
...
export class Type_record_set_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_record_set_bool.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_record_set_bool = new Type_record_set_bool();
