...
export class Type_or_left_record_1_field {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const res = await ex.deploy("./tests/contracts/type_or_left_record_1_field.arl", {}, params);
    ...
    async get_res(): Promise<att.Or<r_record, r_record>> {
        if (this.address != undefined) {
            ...
            return (x => {
                ...
                const value = is_left ? (x => { return (x => { return new att.Nat(x); })(x); })(x["0"]) : (x => { return (x => { return new att.Nat(x); })(x); })(x["1"]);
