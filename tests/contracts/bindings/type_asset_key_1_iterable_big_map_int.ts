...
export class Type_asset_key_1_iterable_big_map_int {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_key_1_iterable_big_map_int.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset_value(key: att.Int): Promise<string | undefined> {
        if (this.address != undefined) {
    ...
    async has_my_asset_value(key: att.Int): Promise<boolean> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            const raw_data = await ex.get_big_map_value(BigInt(att.Int.from_mich((storage as att.Mpair)?.args[0]).toString()), key.to_mich(), my_asset_key_mich_type);
            const data = raw_data ? raw_data?.args[1] : undefined;
...
export const type_asset_key_1_iterable_big_map_int = new Type_asset_key_1_iterable_big_map_int();
