...
export const storage_type: att.MichelineType = { prim: "or", args: [{ prim: "pair", args: [{ prim: "nat", annots: ["%f_a"] }, { prim: "pair", args: [{ prim: "pair", args: [{ prim: "string", annots: ["%f_b"] }, { prim: "bytes", annots: ["%f_c"] }], annots: [] }, { prim: "bool", annots: ["%f_d"] }], annots: [] }], annots: [] }, { prim: "nat", annots: [] }], annots: [] };
...
export class Type_or_left_record_4_fields_custom {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_or_left_record_4_fields_custom.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_res(): Promise<att.Or<r_record, att.Nat>> {
        if (this.address != undefined) {
            ...
            return att.mich_to_or(storage, x => { return mich_to_r_record(x, collapsed); }, x => { return att.mich_to_nat(x); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
