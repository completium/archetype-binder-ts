...
export class Type_option_option_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_option_option_string.arl", {}, params)).address;
        this.address = address;
    }
    async set_value(i: att.Option<att.Option<string>>, params: Partial<ex.Parameters>): Promise<att.CallResult> {
    ...
    async get_res(): Promise<att.Option<att.Option<string>>> {
        if (this.address != undefined) {
            ...
            return att.Option.from_mich(storage, x => { return att.Option.from_mich(x, x => { return att.mich_to_string(x); }); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
