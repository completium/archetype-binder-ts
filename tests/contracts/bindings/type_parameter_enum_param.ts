...
const asset_add_arg_to_mich = (i: e_enum): att.Micheline => {
    return i.to_mich();
}
export class Type_parameter_enum_param {
    ...
    async deploy(res: e_enum, params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_parameter_enum_param.arl", {
            res: res.to_mich()
        }, params)).address;
        this.address = address;
    }
    ...
    async get_asset_add_param(i: e_enum, params: Partial<ex.Parameters>): Promise<att.CallParameter> {
        if (this.address != undefined) {
...
export const type_parameter_enum_param = new Type_parameter_enum_param();
