...
export class Type_option_key {
    ...
    async set_value(i: att.Option<att.Key>, params: Partial<ex.Parameters>): Promise<att.CallResult> {
    ...
    async get_res(): Promise<att.Option<att.Key>> {
        if (this.address != undefined) {
            ...
            return att.Option.from_mich(storage, x => { return att.Key.from_mich(x); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
