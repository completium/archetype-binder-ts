...
export const storage_type: att.MichelineType = { prim: "list", args: [{ prim: "bool", annots: [] }], annots: [] };
...
export class Type_getter_list_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_getter_list_bool.arl", {}, params)).address;
        this.address = address;
    ...
    async get_value(i: Array<boolean>, params: Partial<ex.Parameters>): Promise<Array<boolean>> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<Array<boolean>>(this.get_value_callback_address, x => { return att.mich_to_list(x, x => { return att.mich_to_bool(x); }); });
            }
        }
        throw new Error("Contract not initialised");
    }
