...
export const storage_type: att.MichelineType = { prim: "map", args: [{ prim: "nat", annots: [] }, { prim: "bool", annots: [] }], annots: [] };
...
export class Type_map_value_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_map_value_bool.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_res(): Promise<Array<[
        ...
        if (this.address != undefined) {
            ...
            return att.mich_to_map(storage, (x, y) => [att.mich_to_nat(x), att.mich_to_bool(y)]);
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
