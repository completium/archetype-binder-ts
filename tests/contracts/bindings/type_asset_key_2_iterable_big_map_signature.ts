...
export class my_asset_key implements att.ArchetypeType {
    ...
    static from_mich(input: att.Micheline): my_asset_key {
        return new my_asset_key(att.Signature.from_mich((input as att.Mpair).args[0]), att.Nat.from_mich((input as att.Mpair).args[1]));
    }
}
...
export const my_asset_container_mich_type: att.MichelineType = att.pair_array_to_mich_type([
    att.pair_annot_to_mich_type("big_map", att.pair_array_to_mich_type([
        att.prim_annot_to_mich_type("signature", []),
...
export class Type_asset_key_2_iterable_big_map_signature {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_key_2_iterable_big_map_signature.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_key_2_iterable_big_map_signature = new Type_asset_key_2_iterable_big_map_signature();
