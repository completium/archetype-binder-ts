...
export class my_asset_value implements att.ArchetypeType {
    ...
    static from_mich(input: att.Micheline): my_asset_value {
        ...
        })(att.pair_to_mich((input as att.Mpair as att.Mpair).args.slice(1, 5))));
    }
}
...
export class Type_asset_value_3_iterable_big_map_tuple_nat_string_bytes_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_3_iterable_big_map_tuple_nat_string_bytes_bool.arl", {}, params)).address;
        this.address = address;
    }
    async asset_put(i: [
        att.Nat,
    ...
    async get_my_asset_value(key: att.Nat): Promise<my_asset_value | undefined> {
        if (this.address != undefined) {
            ...
            const data = raw_data ? att.pair_to_mich((raw_data as att.Mpair).args.slice(1, 6)) : undefined;
            if (data != undefined) {
...
export const type_asset_value_3_iterable_big_map_tuple_nat_string_bytes_bool = new Type_asset_value_3_iterable_big_map_tuple_nat_string_bytes_bool();
