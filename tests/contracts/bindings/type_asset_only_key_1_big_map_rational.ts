...
export class my_asset_key implements att.ArchetypeType {
    constructor(public k: att.Rational) { }
    toString(): string {
        return JSON.stringify(this, null, 2);
    }
    to_mich(): att.Micheline {
        return att.pair_to_mich([this.k.to_mich(), this.k.to_mich()]);
    }
    equals(v: my_asset_key): boolean {
        return this.k.equals(v.k);
    }
    static from_mich(input: att.Micheline): my_asset_key {
        return new my_asset_key(att.mich_to_rational(input));
    }
}
...
export class Type_asset_only_key_1_big_map_rational {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_only_key_1_big_map_rational.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_only_key_1_big_map_rational = new Type_asset_only_key_1_big_map_rational();
