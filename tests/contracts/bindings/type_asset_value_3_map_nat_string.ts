...
export const mich_to_my_asset_value = (v: att.Micheline, collapsed: boolean = false): my_asset_value => {
    ...
    return new my_asset_value(att.mich_to_string(fields[0]), att.mich_to_map(fields[1], (x, y) => [att.mich_to_nat(x), att.mich_to_string(y)]));
};
...
export class Type_asset_value_3_map_nat_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_asset_value_3_map_nat_string.arl", {}, params);
        this.address = address;
    }
    ...
    async get_my_asset(): Promise<my_asset_container> {
        if (this.address != undefined) {
            ...
            for (let e of storage.entries()) {
                res.push([(x => { return new att.Nat(x); })(e[0]), (x => { return new my_asset_value((x => { return x; })(x.s), (x => { let res: Array<[
                        att.Nat,
                        string
...
export const type_asset_value_3_map_nat_string = new Type_asset_value_3_map_nat_string();
