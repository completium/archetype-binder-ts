...
export class Type_option_sapling_transaction {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_option_sapling_transaction.arl", {}, params)).address;
        this.address = address;
    }
    async set_value(i: att.Option<att.Sapling_transaction>, params: Partial<ex.Parameters>): Promise<att.CallResult> {
    ...
    async get_res(): Promise<att.Option<att.Sapling_transaction>> {
        if (this.address != undefined) {
            ...
            return att.Option.from_mich(storage, x => { return att.Sapling_transaction.from_mich(x); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
