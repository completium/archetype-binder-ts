...
export const storage_type: att.MichelineType = { prim: "map", args: [{ prim: "key_hash", annots: [] }, { prim: "nat", annots: [] }], annots: [] };
...
export class Type_map_key_key_hash {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_map_key_key_hash.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_res(): Promise<Array<[
        ...
        if (this.address != undefined) {
            ...
            return att.mich_to_map(storage, (x, y) => [att.mich_to_key_hash(x), att.mich_to_nat(y)]);
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
