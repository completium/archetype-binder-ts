...
export class my_asset_key implements att.ArchetypeType {
    ...
    to_mich(): att.Micheline {
        return att.pair_to_mich([this.k.to_mich(), att.string_to_mich(this.k), this.k.to_mich(), att.bool_to_mich(this.k)]);
    }
...
export class Type_asset_only_key_1_big_map_tuple_nat_string_bytes_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_only_key_1_big_map_tuple_nat_string_bytes_bool.arl", {}, params)).address;
        this.address = address;
    }
    async asset_put(i: [
        att.Nat,
    ...
    async get_my_asset_value(key: my_asset_key): Promise<my_asset_value | undefined> {
        if (this.address != undefined) {
            ...
            const data = await ex.get_big_map_value(BigInt(att.mich_to_int(storage).toString()), att.pair_to_mich([key[0].to_mich(), att.string_to_mich(key[1]), key[2].to_mich(), att.bool_to_mich(key[3])]), my_asset_key_mich_type);
            if (data != undefined) {
...
export const type_asset_only_key_1_big_map_tuple_nat_string_bytes_bool = new Type_asset_only_key_1_big_map_tuple_nat_string_bytes_bool();
