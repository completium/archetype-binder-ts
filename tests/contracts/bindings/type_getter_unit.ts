...
export class Type_getter_unit {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_getter_unit.arl", {}, params);
        this.address = address;
    ...
    async get_value(i: att.Unit, params: Partial<ex.Parameters>): Promise<att.Unit> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<att.Unit>(this.get_value_callback_address, x => { return new att.Unit(x); });
            }
        }
        throw new Error("Contract not initialised");
    }
