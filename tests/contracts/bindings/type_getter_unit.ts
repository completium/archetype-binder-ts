...
export const deploy_get_value_callback = async (params: Partial<ex.Parameters>): Promise<att.DeployResult> => {
    return await ex.deploy_callback("get_value", att.prim_annot_to_mich_type("unit", []), params);
};
    ...
    async get_value(i: att.Unit, params: Partial<ex.Parameters>): Promise<att.Unit> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<att.Unit>(this.get_value_callback_address, x => { return new att.Unit(); });
            }
        }
        throw new Error("Contract not initialised");
    }
