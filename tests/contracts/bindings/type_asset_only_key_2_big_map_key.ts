...
export class my_asset_key implements att.ArchetypeType {
    ...
    static from_mich(input: att.Micheline): my_asset_key {
        return new my_asset_key(att.mich_to_key((input as att.Mpair).args[0]), att.mich_to_nat((input as att.Mpair).args[1]));
    }
}
...
export class Type_asset_only_key_2_big_map_key {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_only_key_2_big_map_key.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_only_key_2_big_map_key = new Type_asset_only_key_2_big_map_key();
