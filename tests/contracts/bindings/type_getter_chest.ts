...
export const deploy_get_value_callback = async (params: Partial<ex.Parameters>): Promise<att.DeployResult> => {
    return await ex.deploy_callback("get_value", att.prim_annot_to_mich_type("chest", []), params);
};
export class Type_getter_chest {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_getter_chest.arl", {}, params)).address;
        this.address = address;
    ...
    async get_value(i: att.Chest, params: Partial<ex.Parameters>): Promise<att.Chest> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<att.Chest>(this.get_value_callback_address, x => { return att.Chest.from_mich(x); });
            }
        }
        throw new Error("Contract not initialised");
    }
