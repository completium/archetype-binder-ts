...
export const storage_type: att.MichelineType = { prim: "or", args: [{ prim: "nat", annots: [] }, { prim: "pair", args: [{ prim: "nat", annots: ["%f_a"] }, { prim: "pair", args: [{ prim: "pair", args: [{ prim: "string", annots: ["%f_b"] }, { prim: "bytes", annots: ["%f_c"] }], annots: [] }, { prim: "bool", annots: ["%f_d"] }], annots: [] }], annots: [] }], annots: [] };
...
export class Type_or_right_record_4_fields_custom {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_or_right_record_4_fields_custom.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_res(): Promise<att.Or<att.Nat, r_record>> {
        if (this.address != undefined) {
            ...
            return att.mich_to_or(storage, x => { return att.mich_to_nat(x); }, x => { return mich_to_r_record(x, collapsed); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
}
export const type_or_right_record_4_fields_custom = new Type_or_right_record_4_fields_custom();
