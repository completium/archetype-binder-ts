...
export const storage_type: att.MichelineType = { prim: "or", args: [{ prim: "nat", annots: [] }, { prim: "bool", annots: [] }], annots: [] };
...
export class Type_or_right_bool {
    ...
    async get_res(): Promise<att.Or<att.Nat, boolean>> {
        if (this.address != undefined) {
            ...
            return att.mich_to_or(storage, x => { return att.mich_to_nat(x); }, x => { return att.mich_to_bool(x); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
