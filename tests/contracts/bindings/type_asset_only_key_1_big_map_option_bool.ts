...
export class my_asset_key implements att.ArchetypeType {
    constructor(public k: att.Option<boolean>) { }
    toString(): string {
        return JSON.stringify(this, null, 2);
    }
    to_mich(): att.Micheline {
        return this.k.to_mich((x => { return att.bool_to_mich(x); }));
    }
    equals(v: my_asset_key): boolean {
        return this.k.equals(v.k);
    }
    static from_mich(input: att.Micheline): my_asset_key {
        return new my_asset_key(att.mich_to_option(input, x => { return att.mich_to_bool(x); }));
    }
}
...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("big_map", att.option_annot_to_mich_type(att.prim_annot_to_mich_type("bool", []), []), att.prim_annot_to_mich_type("unit", []), []);
const asset_put_arg_to_mich = (i: att.Option<boolean>): att.Micheline => {
    return i.to_mich((x => { return att.bool_to_mich(x); }));
}
export class Type_asset_only_key_1_big_map_option_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_only_key_1_big_map_option_bool.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset_value(key: my_asset_key): Promise<my_asset_value | undefined> {
        if (this.address != undefined) {
            ...
            const data = await ex.get_big_map_value(BigInt(att.mich_to_int(storage).toString()), key.to_mich((x => { return att.bool_to_mich(x); })), my_asset_key_mich_type);
            if (data != undefined) {
...
export const type_asset_only_key_1_big_map_option_bool = new Type_asset_only_key_1_big_map_option_bool();
