...
export class my_asset_key implements att.ArchetypeType {
    constructor(public k: string, public n: att.Nat) { }
    toString(): string {
        return JSON.stringify(this, null, 2);
    }
    to_mich(): att.Micheline {
        return att.pair_to_mich([att.string_to_mich(this.k), this.n.to_mich()]);
    }
    equals(v: my_asset_key): boolean {
        return att.micheline_equals(this.to_mich(), v.to_mich());
    }
    static from_mich(input: att.Micheline): my_asset_key {
        return new my_asset_key(att.mich_to_string((input as att.Mpair).args[0]), att.mich_to_nat((input as att.Mpair).args[1]));
    }
}
export const my_asset_key_mich_type: att.MichelineType = att.pair_array_to_mich_type([
    att.prim_annot_to_mich_type("string", ["%k"]),
...
export class Type_asset_only_key_2_big_map_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_only_key_2_big_map_string.arl", {}, params)).address;
        this.address = address;
    }
    async asset_put(i: string, params: Partial<ex.Parameters>): Promise<any> {
...
export const type_asset_only_key_2_big_map_string = new Type_asset_only_key_2_big_map_string();
