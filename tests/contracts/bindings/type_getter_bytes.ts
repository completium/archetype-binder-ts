...
export class Type_getter_bytes {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_getter_bytes.arl", {}, params);
        this.address = address;
    ...
    async get_value(i: att.Bytes, params: Partial<ex.Parameters>): Promise<att.Bytes> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<att.Bytes>(this.get_value_callback_address, x => { });
            }
        }
        throw new Error("Contract not initialised");
    }
