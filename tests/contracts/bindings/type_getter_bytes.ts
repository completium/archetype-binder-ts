...
export const deploy_get_value_callback = async (): Promise<att.DeployResult> => {
    return await ex.deploy_callback("get_value", att.prim_annot_to_mich_type("bytes", []));
};
    ...
    async get_value(i: att.Bytes, params: Partial<ex.Parameters>): Promise<att.Bytes> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<att.Bytes>(this.get_value_callback_address, x => { return att.mich_to_bytes(x); });
            }
        }
        throw new Error("Contract not initialised");
    }
