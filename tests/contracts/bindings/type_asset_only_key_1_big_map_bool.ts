...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("big_map", att.prim_annot_to_mich_type("bool", []), att.prim_annot_to_mich_type("unit", []), []);
const asset_put_arg_to_mich = (i: boolean): att.Micheline => {
    return att.bool_to_mich(i);
}
export class Type_asset_only_key_1_big_map_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_only_key_1_big_map_bool.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset_value(key: boolean): Promise<my_asset_value | undefined> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            const data = await ex.get_big_map_value(BigInt(att.Int.from_mich(storage).toString()), att.bool_to_mich(key), my_asset_key_mich_type);
            if (data != undefined) {
...
export const type_asset_only_key_1_big_map_bool = new Type_asset_only_key_1_big_map_bool();
