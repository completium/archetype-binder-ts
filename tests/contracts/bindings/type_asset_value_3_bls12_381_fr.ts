...
export class my_asset_value implements att.ArchetypeType {
    constructor(public s: string, public v: att.Bls12_381_fr) { }
    toString(): string {
...
export const my_asset_value_mich_type: att.MichelineType = att.pair_array_to_mich_type([
    ...
    att.prim_annot_to_mich_type("bls12_381_fr", ["%v"])
], []);
export const mich_to_my_asset_value = (v: att.Micheline, collapsed: boolean = false): my_asset_value => {
    ...
    return new my_asset_value(att.mich_to_string(fields[0]), att.mich_to_bls12_381_fr(fields[1]));
};
...
export class Type_asset_value_3_bls12_381_fr {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_asset_value_3_bls12_381_fr.arl", {}, params);
        this.address = address;
    }
    ...
    async get_my_asset(): Promise<my_asset_container> {
        if (this.address != undefined) {
            ...
            for (let e of storage.entries()) {
                res.push([(x => { return new att.Nat(x); })(e[0]), (x => { return new my_asset_value((x => { return x; })(x.s), (x => { return new att.Bls12_381_fr(x); })(x.v)); })(e[1])]);
            }
            return res;
        }
        throw new Error("Contract not initialised");
...
export const type_asset_value_3_bls12_381_fr = new Type_asset_value_3_bls12_381_fr();
