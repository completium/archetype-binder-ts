...
export class Type_or_right_record_2_fields {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_or_right_record_2_fields.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res(): Promise<att.Or<att.Nat, r_record>> {
        if (this.address != undefined) {
            ...
            return (x => {
                ...
                const value = is_left ? (x => { return new att.Nat(x["0"]); })(x["0"]) : (x => { return new r_record((x => { return new att.Nat(x); })(x["1"].f_a), (x => { return x; })(x["1"].f_b)); })(x["1"]);
