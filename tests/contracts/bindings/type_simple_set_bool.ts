...
export const storage_type: att.MichelineType = { prim: "set", args: [{ prim: "bool", annots: [] }], annots: [] };
const set_value_arg_to_mich = (i: Array<boolean>): att.Micheline => {
    return att.list_to_mich(i, x => {
        return att.bool_to_mich(x);
    });
}
export class Type_simple_set_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_simple_set_bool.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_simple_set_bool = new Type_simple_set_bool();
