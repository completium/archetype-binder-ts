...
export class Type_big_map_key_tuple_nat_string_bytes_bool {
    ...
    async get_res_value(key: [
        ...
        if (this.address != undefined) {
            ...
            const data = await ex.get_big_map_value(BigInt(att.mich_to_int(storage)), att.pair_to_mich([key[0].to_mich(), att.string_to_mich(key[1]), key[2].to_mich(), att.bool_to_mich(key[3])]), att.pair_array_to_mich_type([
