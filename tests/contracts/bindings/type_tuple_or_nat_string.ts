...
export class Type_tuple_or_nat_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_tuple_or_nat_string.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res(): Promise<[
        ...
        if (this.address != undefined) {
            ...
            return (p => {
                ...
                return [att.mich_to_nat(p0.args[0]), att.mich_to_or(p0.args[1]), att.mich_to_string(p1.args[0])];
            })(storage);
        }
        throw new Error("Contract not initialised");
    }
