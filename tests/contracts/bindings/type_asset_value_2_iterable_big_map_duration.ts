...
export class Type_asset_value_2_iterable_big_map_duration {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_2_iterable_big_map_duration.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset_value(key: att.Nat): Promise<att.Duration | undefined> {
        if (this.address != undefined) {
            ...
            if (data != undefined) {
                return att.Duration.from_mich(data);
            }
            else {
                return undefined;
            }
...
export const type_asset_value_2_iterable_big_map_duration = new Type_asset_value_2_iterable_big_map_duration();
