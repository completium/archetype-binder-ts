...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("map", att.prim_annot_to_mich_type("nat", []), att.or_to_mich_type(att.prim_annot_to_mich_type("nat", []), att.prim_annot_to_mich_type("string", []), []), []);
...
export class Type_asset_value_2_or_nat_string {
    ...
    async get_my_asset(): Promise<my_asset_container> {
        if (this.address != undefined) {
            ...
            return att.mich_to_map(storage, (x, y) => [att.Nat.from_mich(x), att.Or.from_mich(y, x => { return att.Nat.from_mich(x); }, x => { return att.mich_to_string(x); })]);
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
