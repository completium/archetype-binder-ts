...
export const storage_type: att.MichelineType = { prim: "option", args: [{ prim: "list", args: [{ prim: "nat", annots: [] }], annots: [] }], annots: [] };
const set_value_arg_to_mich = (i: att.Option<Array<att.Nat>>): att.Micheline => {
    return i.to_mich((x => { return att.list_to_mich(x, x => {
        return x.to_mich();
    }); }));
}
export class Type_option_list_nat {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_option_list_nat.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_option_list_nat = new Type_option_list_nat();
