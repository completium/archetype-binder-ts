...
export const my_asset_container_mich_type: att.MichelineType = att.pair_array_to_mich_type([
    att.pair_annot_to_mich_type("big_map", att.prim_annot_to_mich_type("nat", []), att.pair_array_to_mich_type([
        ...
        att.prim_annot_to_mich_type("sapling_transaction", ["%value"])
    ], ["%values"]), ["%values"]),
...
export class Type_asset_value_2_iterable_big_map_sapling_transaction {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_2_iterable_big_map_sapling_transaction.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset_value(key: att.Nat): Promise<att.Sapling_transaction | undefined> {
        if (this.address != undefined) {
            ...
            if (data != undefined) {
                return att.Sapling_transaction.from_mich(data);
            }
            else {
                return undefined;
            }
...
export const type_asset_value_2_iterable_big_map_sapling_transaction = new Type_asset_value_2_iterable_big_map_sapling_transaction();
