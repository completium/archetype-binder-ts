...
export class my_asset_value implements att.ArchetypeType {
    ...
    static from_mich(input: att.Micheline): my_asset_value {
        return new my_asset_value(att.mich_to_string((input as att.Mpair).args[0]), att.Option.from_mich((input as att.Mpair).args[1], x => { return att.Nat.from_mich(x); }));
    }
}
...
export class Type_asset_value_3_iterable_big_map_option_nat {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_3_iterable_big_map_option_nat.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_value_3_iterable_big_map_option_nat = new Type_asset_value_3_iterable_big_map_option_nat();
