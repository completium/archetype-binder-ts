...
export class my_record implements att.ArchetypeType {
    ...
    to_mich(): att.Micheline {
        return att.pair_to_mich([this.n.to_mich(), att.pair_to_mich([att.pair_to_mich([this.v[0].to_mich(), att.string_to_mich(this.v[1]), this.v[2].to_mich(), att.bool_to_mich(this.v[3])]), att.string_to_mich(this.s)])]);
    }
    equals(v: my_record): boolean {
        return (this.n.equals(v.n) && this.n.equals(v.n) && ((x, y) => {
            return x[0].equals(y[0]) && x[1] == y[1] && x[2].equals(y[2]) && x[3] == y[3];
...
export class Type_record_tuple_nat_string_bytes_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_record_tuple_nat_string_bytes_bool.arl", {}, params);
        this.address = address;
    }
...
export const type_record_tuple_nat_string_bytes_bool = new Type_record_tuple_nat_string_bytes_bool();
