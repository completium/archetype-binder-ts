...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("map", att.or_to_mich_type(att.prim_annot_to_mich_type("nat", []), att.prim_annot_to_mich_type("string", []), []), att.prim_annot_to_mich_type("string", []), []);
...
export class Type_asset_key_1_or_nat_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_key_1_or_nat_string.arl", {}, params)).address;
        this.address = address;
    }
