...
export class Type_big_map_value_enum_simple {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_big_map_value_enum_simple.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res_value(key: att.Nat): Promise<enum_simple | undefined> {
        if (this.address != undefined) {
            ...
            if (data != undefined) {
                if (data.e_3 !== undefined) {
                    return new e_3();
                }
                else if (data.e_2 !== undefined) {
                    return new e_2();
                }
                else
                    return new e_1();
