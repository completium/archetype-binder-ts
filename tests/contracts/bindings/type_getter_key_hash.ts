    ...
    async get_value(i: att.Key_hash, params: Partial<ex.Parameters>): Promise<att.Key_hash> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<att.Key_hash>(this.get_value_callback_address, x => { return att.mich_to_key_hash(x); });
            }
        }
        throw new Error("Contract not initialised");
    }
...
export const type_getter_key_hash = new Type_getter_key_hash();
