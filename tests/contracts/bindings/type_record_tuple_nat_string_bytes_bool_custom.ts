...
export class my_record implements att.ArchetypeType {
    ...
    to_mich(): att.Micheline {
        return att.pair_to_mich([this.n.to_mich(), att.pair_to_mich([att.pair_to_mich([this.v[0].to_mich(), att.pair_to_mich([att.string_to_mich(this.v[1][0]), this.v[1][1].to_mich()]), att.bool_to_mich(this.v[2])]), att.string_to_mich(this.s)])]);
    }
...
export class Type_record_tuple_nat_string_bytes_bool_custom {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_record_tuple_nat_string_bytes_bool_custom.arl", {}, params);
        this.address = address;
    }
