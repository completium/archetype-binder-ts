...
export class Type_big_map_key_or_nat_string {
    ...
    async get_res_value(key: att.Or<att.Nat, string>): Promise<att.Nat | undefined> {
        if (this.address != undefined) {
            ...
            const data = await ex.get_big_map_value(BigInt(storage), key.to_mich(), att.or_to_mich_type(att.prim_annot_to_mich_type("nat", []), att.prim_annot_to_mich_type("string", []), []), att.prim_annot_to_mich_type("nat", [])), collapsed = true;
            if (data != undefined) {
