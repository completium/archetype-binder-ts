...
import { mich_to_or_value } from "@completium/archetype-ts-types";
...
export class Type_simple_or_nat_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_simple_or_nat_string.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res(): Promise<att.Or<att.Nat, string>> {
        if (this.address != undefined) {
            ...
            return new att.Or<att.Nat, string>("", false);
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
