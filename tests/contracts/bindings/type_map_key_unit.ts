...
export const storage_type: att.MichelineType = { prim: "map", args: [{ prim: "unit", annots: [] }, { prim: "nat", annots: [] }], annots: [] };
const set_value_arg_to_mich = (i: att.Unit): att.Micheline => {
    return att.unit_to_mich();
}
export class Type_map_key_unit {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_map_key_unit.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_map_key_unit = new Type_map_key_unit();
