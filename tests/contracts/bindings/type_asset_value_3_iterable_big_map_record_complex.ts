...
export class my_asset_value implements att.ArchetypeType {
    ...
    static from_mich(input: att.Micheline): my_asset_value {
        return new my_asset_value(att.mich_to_string((input as att.Mpair).args[0]), r_record.from_mich(att.pair_to_mich((input as att.Mpair as att.Mpair).args.slice(1, 11))));
    }
}
...
export class Type_asset_value_3_iterable_big_map_record_complex {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_3_iterable_big_map_record_complex.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset_value(key: att.Nat): Promise<my_asset_value | undefined> {
        if (this.address != undefined) {
            ...
            const data = raw_data ? att.pair_to_mich((raw_data as att.Mpair).args.slice(1, 12)) : undefined;
            if (data != undefined) {
...
export const type_asset_value_3_iterable_big_map_record_complex = new Type_asset_value_3_iterable_big_map_record_complex();
