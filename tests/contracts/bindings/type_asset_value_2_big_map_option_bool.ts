...
export class my_asset_value implements att.ArchetypeType {
    constructor(public v: att.Option<boolean>) { }
    toString(): string {
        return JSON.stringify(this, null, 2);
    }
    to_mich(): att.Micheline {
        return this.v.to_mich((x => { return att.bool_to_mich(x); }));
    }
    equals(v: my_asset_value): boolean {
        return this.v.equals(v.v);
    }
    static from_mich(input: att.Micheline): my_asset_value {
        return new my_asset_value(att.mich_to_option(input, x => { return att.mich_to_bool(x); }));
    }
}
...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("big_map", att.prim_annot_to_mich_type("nat", []), att.option_annot_to_mich_type(att.prim_annot_to_mich_type("bool", []), []), []);
const asset_put_arg_to_mich = (i: att.Option<boolean>): att.Micheline => {
    return i.to_mich((x => { return att.bool_to_mich(x); }));
}
export class Type_asset_value_2_big_map_option_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_2_big_map_option_bool.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_value_2_big_map_option_bool = new Type_asset_value_2_big_map_option_bool();
