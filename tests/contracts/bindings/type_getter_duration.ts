...
export class Type_getter_duration {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_getter_duration.arl", {}, params)).address;
        this.address = address;
    ...
    async get_value(i: att.Duration, params: Partial<ex.Parameters>): Promise<att.Duration> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<att.Duration>(this.get_value_callback_address, x => { return att.mich_to_duration(x); });
            }
        }
        throw new Error("Contract not initialised");
    }
