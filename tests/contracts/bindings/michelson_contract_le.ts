...
export class Michelson_contract_le {
    ...
    async originate(a: att.Nat, b: string, c: att.Bytes, d: boolean, params: Partial<ex.Parameters>) {
        const address = (await ex.originate("./tests/contracts/michelson_contract_le.tz", storage_arg_to_mich(a, b, c, d), params)).address;
        this.address = address;
    }
    ...
    async get_a(): Promise<att.Nat> {
        if (this.address != undefined) {
            ...
            return att.Nat.from_mich((((storage as att.Mpair).args[0] as att.Mpair).args[0] as att.Mpair).args[0]);
        }
        throw new Error("Contract not initialised");
    }
    async get_b(): Promise<string> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            return att.mich_to_string((((storage as att.Mpair).args[0] as att.Mpair).args[0] as att.Mpair).args[1]);
        }
        throw new Error("Contract not initialised");
    }
    async get_c(): Promise<att.Bytes> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            return att.Bytes.from_mich(((storage as att.Mpair).args[0] as att.Mpair).args[1]);
        }
        throw new Error("Contract not initialised");
    }
...
export const michelson_contract_le = new Michelson_contract_le();
