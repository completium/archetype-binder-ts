...
export class Michelson_contract_le {
    ...
    async get_a(): Promise<att.Nat> {
        if (this.address != undefined) {
            ...
            return att.mich_to_nat((((storage as att.Mpair).args[0] as att.Mpair).args[0] as att.Mpair).args[0]);
        }
        throw new Error("Contract not initialised");
    }
    ...
    async get_c(): Promise<att.Bytes> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            return att.mich_to_bytes(((storage as att.Mpair).args[0] as att.Mpair).args[1]);
        }
        throw new Error("Contract not initialised");
    }
