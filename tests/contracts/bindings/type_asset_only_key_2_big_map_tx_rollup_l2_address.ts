...
export class my_asset_key implements att.ArchetypeType {
    constructor(public k: att.Tx_rollup_l2_address, public n: att.Nat) { }
    toString(): string {
    ...
    static from_mich(input: att.Micheline): my_asset_key {
        return new my_asset_key(att.mich_to_tx_rollup_l2_address((input as att.Mpair).args[0]), att.mich_to_nat((input as att.Mpair).args[1]));
    }
}
export const my_asset_key_mich_type: att.MichelineType = att.pair_array_to_mich_type([
    att.prim_annot_to_mich_type("tx_rollup_l2_address", ["%k"]),
...
export class Type_asset_only_key_2_big_map_tx_rollup_l2_address {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_only_key_2_big_map_tx_rollup_l2_address.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_only_key_2_big_map_tx_rollup_l2_address = new Type_asset_only_key_2_big_map_tx_rollup_l2_address();
