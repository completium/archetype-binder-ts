...
export class my_asset_value implements att.ArchetypeType {
    ...
    static from_mich(input: att.Micheline): my_asset_value {
        return new my_asset_value(att.mich_to_int(input));
    }
}
...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("map", att.prim_annot_to_mich_type("nat", []), att.prim_annot_to_mich_type("int", []), []);
const asset_put_arg_to_mich = (i: att.Int): att.Micheline => {
    return i.to_mich();
}
export class Type_asset_value_2_int {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_2_int.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_value_2_int = new Type_asset_value_2_int();
