...
export const storage_type: att.MichelineType = { prim: "list", args: [{ prim: "map", args: [{ prim: "nat", annots: [] }, { prim: "string", annots: [] }], annots: [] }], annots: [] };
...
export class Type_list_map_nat_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_list_map_nat_string.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_res(): Promise<Array<Array<[
        ...
        if (this.address != undefined) {
            ...
            return att.mich_to_list(storage, x => { return att.mich_to_map(x, (x, y) => [att.mich_to_nat(x), att.mich_to_string(y)]); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
