...
export class Type_view_record_4_fields_custom {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_view_record_4_fields_custom.arl", {}, params)).address;
        this.address = address;
    }
    async view_get_value(i: r_record, params: Partial<ex.Parameters>): Promise<r_record> {
        if (this.address != undefined) {
            const mich = await ex.exec_view(this.get_address(), "get_value", view_get_value_arg_to_mich(i), params);
            return new r_record((x => { return new att.Nat(x); })(mich.value.f_a), (x => { return x; })(mich.value.f_b), (x => { return new att.Bytes(x); })(mich.value.f_c), (x => { return x.prim ? (x.prim == "True" ? true : false) : x; })(mich.value.f_d));
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
}
export const type_view_record_4_fields_custom = new Type_view_record_4_fields_custom();
