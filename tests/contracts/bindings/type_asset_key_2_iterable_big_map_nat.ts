...
export class my_asset_key implements att.ArchetypeType {
    constructor(public k: att.Nat, public n: att.Nat) { }
    toString(): string {
    ...
    static from_mich(input: att.Micheline): my_asset_key {
        return new my_asset_key(att.Nat.from_mich((input as att.Mpair).args[0]), att.Nat.from_mich((input as att.Mpair).args[1]));
    }
}
...
export class Type_asset_key_2_iterable_big_map_nat {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_key_2_iterable_big_map_nat.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_key_2_iterable_big_map_nat = new Type_asset_key_2_iterable_big_map_nat();
