...
export const storage_type: att.MichelineType = { prim: "pair", args: [{ prim: "nat", annots: ["%n"] }, { prim: "pair", args: [{ prim: "unit", annots: ["%v"] }, { prim: "string", annots: ["%s"] }], annots: [] }], annots: [] };
export class my_record implements att.ArchetypeType {
    constructor(public n: att.Nat, public v: att.Unit, public s: string) { }
    toString(): string {
        return JSON.stringify(this, null, 2);
    }
    to_mich(): att.Micheline {
        return att.pair_to_mich([this.n.to_mich(), att.pair_to_mich([att.unit_to_mich(), att.string_to_mich(this.s)])]);
    }
...
export class Type_record_unit {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_record_unit.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_record_unit = new Type_record_unit();
