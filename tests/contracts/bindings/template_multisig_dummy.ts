...
export class Template_multisig_dummy {
    ...
    async deploy(owner: att.Address, params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/template_multisig_dummy.arl", {
            owner: owner.to_mich()
        }, params)).address;
        this.address = address;
    }
...
export const template_multisig_dummy = new Template_multisig_dummy();
