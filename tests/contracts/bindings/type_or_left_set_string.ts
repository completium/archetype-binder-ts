...
export class Type_or_left_set_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_or_left_set_string.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res(): Promise<att.Or<Array<string>, att.Nat>> {
        if (this.address != undefined) {
            ...
            return (x => {
                ...
                const value = is_left ? (x => { const res: Array<string> = []; for (let i = 0; i < x["0"].length; i++) {
