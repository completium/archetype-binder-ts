...
export const mich_to_my_asset_value = (v: att.Micheline, collapsed: boolean = false): my_asset_value => {
    ...
    return new my_asset_value(att.mich_to_string(fields[0]), att.mich_to_nat(fields[1]));
};
...
export class Type_asset_value_3_nat {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_asset_value_3_nat.arl", {}, params);
        this.address = address;
    }
    ...
    async get_my_asset(): Promise<my_asset_container> {
        if (this.address != undefined) {
            ...
            for (let e of storage.entries()) {
                res.push([(x => { return new att.Nat(x); })(e[0]), (x => { return new my_asset_value((x => { return x; })(x.s), (x => { return new att.Nat(x); })(x.v)); })(e[1])]);
            }
            return res;
        }
        throw new Error("Contract not initialised");
...
export const type_asset_value_3_nat = new Type_asset_value_3_nat();
