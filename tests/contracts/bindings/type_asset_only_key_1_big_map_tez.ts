...
export class my_asset_key implements att.ArchetypeType {
    ...
    static from_mich(input: att.Micheline): my_asset_key {
        return new my_asset_key(att.mich_to_tez(input));
    }
}
...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("big_map", att.prim_annot_to_mich_type("mutez", []), att.prim_annot_to_mich_type("unit", []), []);
const asset_put_arg_to_mich = (i: att.Tez): att.Micheline => {
    return i.to_mich();
}
export class Type_asset_only_key_1_big_map_tez {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_only_key_1_big_map_tez.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_only_key_1_big_map_tez = new Type_asset_only_key_1_big_map_tez();
