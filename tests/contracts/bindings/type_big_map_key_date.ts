...
export class Type_big_map_key_date {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_big_map_key_date.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res_value(key: Date): Promise<att.Nat | undefined> {
        if (this.address != undefined) {
            ...
            const data = await ex.get_big_map_value(BigInt(storage.res), att.date_to_mich(key), att.prim_annot_to_mich_type("date", [])), collapsed = true;
            if (data != undefined) {
