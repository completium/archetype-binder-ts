...
export class Type_big_map_key_date {
    ...
    async get_res_value(key: Date): Promise<att.Nat | undefined> {
        if (this.address != undefined) {
            ...
            const data = await ex.get_big_map_value(BigInt(storage), att.date_to_mich(key), att.prim_annot_to_mich_type("timestamp", []), att.prim_annot_to_mich_type("nat", [])), collapsed = true;
            if (data != undefined) {
