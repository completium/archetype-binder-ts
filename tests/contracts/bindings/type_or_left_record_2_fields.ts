...
export class Type_or_left_record_2_fields {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_or_left_record_2_fields.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res(): Promise<att.Or<r_record, att.Nat>> {
        if (this.address != undefined) {
            ...
            return (x => {
                ...
                const value = is_left ? (x => { return new r_record((x => { return new att.Nat(x); })(x["0"].f_a), (x => { return x; })(x["0"].f_b)); })(x["0"]) : (x => { return new att.Nat(x["1"]); })(x["1"]);
