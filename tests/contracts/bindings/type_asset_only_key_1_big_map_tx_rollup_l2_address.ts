...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("big_map", att.prim_annot_to_mich_type("tx_rollup_l2_address", []), att.prim_annot_to_mich_type("unit", []), []);
const asset_put_arg_to_mich = (i: att.Tx_rollup_l2_address): att.Micheline => {
    return i.to_mich();
}
export class Type_asset_only_key_1_big_map_tx_rollup_l2_address {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_only_key_1_big_map_tx_rollup_l2_address.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset_value(key: att.Tx_rollup_l2_address): Promise<my_asset_value | undefined> {
        if (this.address != undefined) {
    ...
    async has_my_asset_value(key: att.Tx_rollup_l2_address): Promise<boolean> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            const data = await ex.get_big_map_value(BigInt(att.mich_to_int(storage).toString()), key.to_mich(), my_asset_key_mich_type);
            if (data != undefined) {
...
export const type_asset_only_key_1_big_map_tx_rollup_l2_address = new Type_asset_only_key_1_big_map_tx_rollup_l2_address();
