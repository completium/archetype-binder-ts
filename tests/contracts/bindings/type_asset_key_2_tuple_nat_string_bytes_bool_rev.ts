...
export class my_asset_key implements att.ArchetypeType {
    ...
    to_mich(): att.Micheline {
        return att.pair_to_mich([att.pair_to_mich([att.pair_to_mich([att.pair_to_mich([this.k[0][0][0].to_mich(), att.string_to_mich(this.k[0][0][1])]), this.k[0][1].to_mich()]), att.bool_to_mich(this.k[1])]), this.n.to_mich()]);
    }
...
export class Type_asset_key_2_tuple_nat_string_bytes_bool_rev {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_key_2_tuple_nat_string_bytes_bool_rev.arl", {}, params)).address;
        this.address = address;
    }
    async asset_put(i: [
        [
    ...
    async get_my_asset(): Promise<my_asset_container> {
        if (this.address != undefined) {
            ...
            for (let e of storage.entries()) {
                res.push([(x => { return new my_asset_key((x => { return [[[(x => { return new att.Nat(x); })(x[Object.keys(x)[0]]), (x => { return x; })(x[Object.keys(x)[1]])], (x => { return new att.Bytes(x); })(x[Object.keys(x)[2]])], (x => { return x.prim ? (x.prim == "True" ? true : false) : x; })(x[Object.keys(x)[3]])]; })(x[0]), (x => { return new att.Nat(x); })(x[1])); })(e[0]), (x => { return x; })(e[1])]);
            }
            return res;
        }
        throw new Error("Contract not initialised");
...
export const type_asset_key_2_tuple_nat_string_bytes_bool_rev = new Type_asset_key_2_tuple_nat_string_bytes_bool_rev();
