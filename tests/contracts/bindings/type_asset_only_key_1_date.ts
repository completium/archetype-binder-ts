...
export type my_asset_container = Array<Date>;
export const my_asset_container_mich_type: att.MichelineType = att.set_annot_to_mich_type(att.prim_annot_to_mich_type("timestamp", []), []);
const asset_put_arg_to_mich = (i: Date): att.Micheline => {
    return att.date_to_mich(i);
}
export class Type_asset_only_key_1_date {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_only_key_1_date.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_only_key_1_date = new Type_asset_only_key_1_date();
