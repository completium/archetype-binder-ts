...
export class Type_asset_value_2_iterable_big_map_unit {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_2_iterable_big_map_unit.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset_value(key: att.Nat): Promise<att.Unit | undefined> {
        if (this.address != undefined) {
...
export const type_asset_value_2_iterable_big_map_unit = new Type_asset_value_2_iterable_big_map_unit();
