    ...
    async get_my_asset(): Promise<my_asset_container> {
        if (this.address != undefined) {
            ...
            for (let e of storage.entries()) {
                res.push([(x => { return new my_asset_key((x => { return new r_record((x => { return new att.Nat(x); })(x[0]), (x => { return x; })(x[1]), (x => { return new att.Bytes(x); })(x[2])); })(x[0]), (x => { return new att.Nat(x); })(x[1])); })(e[0]), (x => { return x; })(e[1])]);
            }
            return res;
        }
        throw new Error("Contract not initialised");
