...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("big_map", att.prim_annot_to_mich_type("nat", []), att.or_to_mich_type(att.prim_annot_to_mich_type("nat", []), att.prim_annot_to_mich_type("string", []), []), []);
...
export class Type_asset_value_2_big_map_or_nat_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_2_big_map_or_nat_string.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset_value(key: att.Nat): Promise<att.Or<att.Nat, string> | undefined> {
        if (this.address != undefined) {
            ...
            if (data != undefined) {
                return att.Or.from_mich(data, x => { return att.Nat.from_mich(x); }, x => { return att.mich_to_string(x); });
            }
            else {
                return undefined;
            }
...
export const type_asset_value_2_big_map_or_nat_string = new Type_asset_value_2_big_map_or_nat_string();
