...
export class my_asset_value implements att.ArchetypeType {
    ...
    static from_mich(input: att.Micheline): my_asset_value {
        return new my_asset_value(att.mich_to_string((input as att.Mpair).args[0]), att.mich_to_or((input as att.Mpair).args[1], x => { return att.mich_to_nat(x); }, x => { return att.mich_to_string(x); }));
    }
}
...
export class Type_asset_value_3_big_map_or_nat_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_3_big_map_or_nat_string.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_value_3_big_map_or_nat_string = new Type_asset_value_3_big_map_or_nat_string();
