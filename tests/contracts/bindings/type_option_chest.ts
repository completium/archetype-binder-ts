...
export class Type_option_chest {
    ...
    async set_value(i: att.Option<att.Chest>, params: Partial<ex.Parameters>): Promise<att.CallResult> {
    ...
    async get_res(): Promise<att.Option<att.Chest>> {
        if (this.address != undefined) {
            ...
            return att.Option.from_mich(storage, x => { return att.Chest.from_mich(x); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
