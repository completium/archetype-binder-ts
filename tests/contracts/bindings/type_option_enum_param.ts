...
export class Type_option_enum_param {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_option_enum_param.arl", {}, params)).address;
        this.address = address;
    }
    async set_value(i: att.Option<e_enum>, params: Partial<ex.Parameters>): Promise<att.CallResult> {
    ...
    async get_res(): Promise<att.Option<e_enum>> {
        if (this.address != undefined) {
            ...
            return att.Option.from_mich(storage, x => { return mich_to_e_enum(x); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
}
export const type_option_enum_param = new Type_option_enum_param();
