...
export class my_asset_value implements att.ArchetypeType {
    constructor(public s: string, public v: Array<[
        att.Nat,
        string
    ]>) { }
    toString(): string {
    ...
    static from_mich(input: att.Micheline): my_asset_value {
        return new my_asset_value(att.mich_to_string((input as att.Mpair).args[0]), att.mich_to_map((input as att.Mpair).args[1], (x, y) => [att.Nat.from_mich(x), att.mich_to_string(y)]));
    }
}
...
export class Type_asset_value_3_iterable_big_map_map_nat_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_3_iterable_big_map_map_nat_string.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_value_3_iterable_big_map_map_nat_string = new Type_asset_value_3_iterable_big_map_map_nat_string();
