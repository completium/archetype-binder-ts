...
export class my_asset_value implements att.ArchetypeType {
    ...
    to_mich(): att.Micheline {
        return att.date_to_mich(this.v);
    }
    equals(v: my_asset_value): boolean {
        return (this.v.getTime() - this.v.getMilliseconds()) == (v.v.getTime() - v.v.getMilliseconds());
    }
    static from_mich(input: att.Micheline): my_asset_value {
        return new my_asset_value(att.mich_to_date(input));
    }
}
...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("big_map", att.prim_annot_to_mich_type("nat", []), att.prim_annot_to_mich_type("timestamp", []), []);
const asset_put_arg_to_mich = (i: Date): att.Micheline => {
    return att.date_to_mich(i);
}
export class Type_asset_value_2_big_map_date {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_2_big_map_date.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_value_2_big_map_date = new Type_asset_value_2_big_map_date();
