...
export const deploy_get_value_callback = async (params: Partial<ex.Parameters>): Promise<att.DeployResult> => {
    return await ex.deploy_callback("get_value", att.set_annot_to_mich_type(att.prim_annot_to_mich_type("string", []), []), params);
};
    ...
    async get_value(i: Array<string>, params: Partial<ex.Parameters>): Promise<Array<string>> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<Array<string>>(this.get_value_callback_address, x => { return att.mich_to_list(x, x => { return att.mich_to_string(x); }); });
            }
        }
        throw new Error("Contract not initialised");
    }
...
export const type_getter_set_string = new Type_getter_set_string();
