...
export class Type_getter_set_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_getter_set_string.arl", {}, params);
        this.address = address;
    ...
    async get_value(i: Array<string>, params: Partial<ex.Parameters>): Promise<Array<string>> {
        if (this.address != undefined) {
            if (this.get_value_callback_address != undefined) {
                ...
                return await ex.get_callback_value<Array<string>>(this.get_value_callback_address, x => { });
            }
        }
        throw new Error("Contract not initialised");
    }
