...
export class my_asset_key implements att.ArchetypeType {
    ...
    static from_mich(input: att.Micheline): my_asset_key {
        return new my_asset_key(att.mich_to_string((input as att.Mpair).args[0]), att.Nat.from_mich((input as att.Mpair).args[1]));
    }
}
...
export class Type_asset_key_2_iterable_big_map_string {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_key_2_iterable_big_map_string.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_asset_key_2_iterable_big_map_string = new Type_asset_key_2_iterable_big_map_string();
