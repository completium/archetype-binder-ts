...
export class Type_map_key_enum_param {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_map_key_enum_param.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_res(): Promise<Array<[
        ...
        if (this.address != undefined) {
            ...
            return att.mich_to_map(storage, (x, y) => [mich_to_e_enum(x), att.Nat.from_mich(y)]);
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
}
export const type_map_key_enum_param = new Type_map_key_enum_param();
