...
export const mich_to_e_enum = (m: att.Micheline): e_enum => {
    ...
    if ((((m as att.Msingle).args[0] as att.Msingle)).prim == "Left") {
        const a = (((m as att.Msingle).args[0] as att.Msingle)).args[0];
        return new e_2(att.mich_to_nat(a))
    }
    if ((((m as att.Msingle).args[0] as att.Msingle)).prim == "Right") {
        const a = (((m as att.Msingle).args[0] as att.Msingle)).args[0];
        return new e_3(att.mich_to_string(a))
    }
    throw new Error("mich_toe_enum : invalid micheline");
};
const set_value_arg_to_mich = (i: e_enum): att.Micheline => {
    return i.to_mich();
}
export class Type_simple_enum_param {
...
export const type_simple_enum_param = new Type_simple_enum_param();
