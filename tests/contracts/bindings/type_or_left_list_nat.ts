...
export class Type_or_left_list_nat {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_or_left_list_nat.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res(): Promise<att.Or<Array<att.Nat>, att.Nat>> {
        if (this.address != undefined) {
            ...
            return (x => {
                ...
                const value = is_left ? (x => { const res: Array<att.Nat> = []; for (let i = 0; i < x["0"].length; i++) {
                    res.push((x => { return new att.Nat(x); })(x["0"][i]));
