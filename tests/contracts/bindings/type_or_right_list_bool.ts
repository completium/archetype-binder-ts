...
export const storage_type: att.MichelineType = { prim: "or", args: [{ prim: "nat", annots: [] }, { prim: "list", args: [{ prim: "bool", annots: [] }], annots: [] }], annots: [] };
const set_value_arg_to_mich = (i: Array<boolean>): att.Micheline => {
    return att.list_to_mich(i, x => {
        return att.bool_to_mich(x);
    });
}
export class Type_or_right_list_bool {
    ...
    async get_res(): Promise<att.Or<att.Nat, Array<boolean>>> {
        if (this.address != undefined) {
            ...
            return att.mich_to_or(storage, x => { return att.mich_to_nat(x); }, x => { return att.mich_to_list(x, x => { return att.mich_to_bool(x); }); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
}
export const type_or_right_list_bool = new Type_or_right_list_bool();
