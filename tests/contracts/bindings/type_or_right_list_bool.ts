...
export class Type_or_right_list_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_or_right_list_bool.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res(): Promise<att.Or<att.Nat, Array<boolean>>> {
        if (this.address != undefined) {
            ...
            return (x => {
                ...
                const value = is_left ? (x => { return new att.Nat(x["0"]); })(x["0"]) : (x => { const res: Array<boolean> = []; for (let i = 0; i < x["1"].length; i++) {
