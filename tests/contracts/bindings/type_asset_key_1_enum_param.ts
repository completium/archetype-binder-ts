...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("map", att.or_to_mich_type(att.prim_annot_to_mich_type("unit", ["%e_1"]), att.or_to_mich_type(att.prim_annot_to_mich_type("nat", ["%e_2"]), att.or_to_mich_type(att.prim_annot_to_mich_type("string", ["%e_3"]), att.pair_array_to_mich_type([
    att.prim_annot_to_mich_type("bytes", []),
    att.prim_annot_to_mich_type("bool", [])
], ["%e_4"]), []), []), []), att.prim_annot_to_mich_type("string", []), []);
const asset_put_arg_to_mich = (i: e_enum): att.Micheline => {
    return i.to_mich();
}
export class Type_asset_key_1_enum_param {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_key_1_enum_param.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset(): Promise<my_asset_container> {
        if (this.address != undefined) {
            ...
            return att.mich_to_map(storage, (x, y) => [mich_to_e_enum(x), att.mich_to_string(y)]);
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
}
export const type_asset_key_1_enum_param = new Type_asset_key_1_enum_param();
