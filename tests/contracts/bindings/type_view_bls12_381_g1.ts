...
export class Type_view_bls12_381_g1 {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_view_bls12_381_g1.arl", {}, params)).address;
        this.address = address;
    }
    async view_get_value(i: att.Bls12_381_g1, params: Partial<ex.Parameters>): Promise<att.Bls12_381_g1> {
        if (this.address != undefined) {
            const mich = await ex.exec_view(this.get_address(), "get_value", view_get_value_arg_to_mich(i), params);
            return att.mich_to_bls12_381_g1(mich.value);
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
}
export const type_view_bls12_381_g1 = new Type_view_bls12_381_g1();
