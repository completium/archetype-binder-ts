...
export class Type_option_set_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_option_set_bool.arl", {}, params)).address;
        this.address = address;
    }
    async set_value(i: att.Option<Array<boolean>>, params: Partial<ex.Parameters>): Promise<att.CallResult> {
    ...
    async get_res(): Promise<att.Option<Array<boolean>>> {
        if (this.address != undefined) {
            ...
            return att.Option.from_mich(storage, x => { return att.mich_to_list(x, x => { return att.mich_to_bool(x); }); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
