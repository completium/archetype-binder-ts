...
export class Type_view_record_2_fields {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_view_record_2_fields.arl", {}, params);
        this.address = address;
    }
    async view_get_value(params: Partial<ex.Parameters>): Promise<r_record> {
        if (this.address != undefined) {
            ...
            return new r_record((x => { return new att.Nat(x); })(mich.f_a), (x => { return x; })(mich.f_b));
        }
        throw new Error("Contract not initialised");
    }
