...
export class Sample_iterable_big_map {
    ...
    async get_v_value(key: att.Int): Promise<att.Bytes | undefined> {
        if (this.address != undefined) {
            ...
            const data = (await ex.get_big_map_value(BigInt(att.mich_to_int((storage as att.Mpair).args[0]).toString()), key.to_mich(), att.prim_annot_to_mich_type("int", [])) as att.Mpair).args[1];
            if (data != undefined) {
    ...
    async has_v_value(key: att.Int): Promise<boolean> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            const data = await ex.get_big_map_value(BigInt(att.mich_to_int((storage as att.Mpair).args[0]).toString()), key.to_mich(), att.prim_annot_to_mich_type("int", []));
            if (data != undefined) {
                return true;
            }
            else {
...
export const sample_iterable_big_map = new Sample_iterable_big_map();
