...
export const my_asset_container_mich_type: att.MichelineType = att.pair_annot_to_mich_type("big_map", att.option_annot_to_mich_type(att.prim_annot_to_mich_type("nat", []), []), att.prim_annot_to_mich_type("unit", []), []);
const asset_put_arg_to_mich = (i: att.Option<att.Nat>): att.Micheline => {
    return i.to_mich((x => { return x.to_mich(); }));
}
export class Type_asset_only_key_1_big_map_option_nat {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_only_key_1_big_map_option_nat.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset_value(key: att.Option<att.Nat>): Promise<my_asset_value | undefined> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            const data = await ex.get_big_map_value(BigInt(att.Int.from_mich(storage).toString()), key.to_mich((x => { return x.to_mich(); })), my_asset_key_mich_type);
            if (data != undefined) {
...
export const type_asset_only_key_1_big_map_option_nat = new Type_asset_only_key_1_big_map_option_nat();
