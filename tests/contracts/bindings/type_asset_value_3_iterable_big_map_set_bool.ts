...
export class my_asset_value implements att.ArchetypeType {
    constructor(public s: string, public v: Array<boolean>) { }
    toString(): string {
    ...
    static from_mich(input: att.Micheline): my_asset_value {
        return new my_asset_value(att.mich_to_string((input as att.Mpair).args[0]), att.mich_to_list((input as att.Mpair).args[1], x => { return att.mich_to_bool(x); }));
    }
}
export const my_asset_value_mich_type: att.MichelineType = att.pair_array_to_mich_type([
    att.prim_annot_to_mich_type("string", ["%s"]),
    att.set_annot_to_mich_type(att.prim_annot_to_mich_type("bool", []), ["%v"])
], []);
...
export class Type_asset_value_3_iterable_big_map_set_bool {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_value_3_iterable_big_map_set_bool.arl", {}, params)).address;
        this.address = address;
    }
    async asset_put(i: Array<boolean>, params: Partial<ex.Parameters>): Promise<att.CallResult> {
...
export const type_asset_value_3_iterable_big_map_set_bool = new Type_asset_value_3_iterable_big_map_set_bool();
