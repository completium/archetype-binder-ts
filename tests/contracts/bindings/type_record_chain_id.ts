...
export class my_record implements att.ArchetypeType {
    constructor(public n: att.Nat, public v: att.Chain_id, public s: string) { }
    toString(): string {
    ...
    static from_mich(input: att.Micheline): my_record {
        return new my_record(att.Nat.from_mich((input as att.Mpair).args[0]), att.Chain_id.from_mich((input as att.Mpair).args[1]), att.mich_to_string((input as att.Mpair).args[2]));
    }
}
...
export class Type_record_chain_id {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_record_chain_id.arl", {}, params)).address;
        this.address = address;
    }
...
export const type_record_chain_id = new Type_record_chain_id();
