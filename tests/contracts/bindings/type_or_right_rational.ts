...
export class Type_or_right_rational {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = await ex.deploy("./tests/contracts/type_or_right_rational.arl", {}, params);
        this.address = address;
    }
    ...
    async get_res(): Promise<att.Or<att.Nat, att.Rational>> {
        if (this.address != undefined) {
            ...
            return (x => {
                ...
                const value = is_left ? (x => { return new att.Nat(x["0"]); })(x["0"]) : (x => { return new att.Rational(x["1"][Object.keys(x["1"])[0]], x["1"][Object.keys(x["1"])[1]]); })(x["1"]);
