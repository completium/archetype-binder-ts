...
export const storage_type: att.MichelineType = { prim: "big_map", args: [{ prim: "pair", args: [{ prim: "int", annots: [] }, { prim: "nat", annots: [] }], annots: [] }, { prim: "nat", annots: [] }], annots: [] };
...
export class Type_big_map_key_rational {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_big_map_key_rational.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_res_value(key: att.Rational): Promise<att.Nat | undefined> {
        if (this.address != undefined) {
            ...
            const data = await ex.get_big_map_value(BigInt(storage), key.to_mich(), att.pair_array_to_mich_type([
