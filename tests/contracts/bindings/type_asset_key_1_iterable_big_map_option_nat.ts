...
export const my_asset_container_mich_type: att.MichelineType = att.pair_array_to_mich_type([
    att.pair_annot_to_mich_type("big_map", att.option_annot_to_mich_type(att.prim_annot_to_mich_type("nat", []), []), att.pair_array_to_mich_type([
        att.prim_annot_to_mich_type("nat", ["%index"]),
        att.prim_annot_to_mich_type("string", ["%value"])
    ], ["%values"]), ["%values"]),
    att.pair_annot_to_mich_type("big_map", att.prim_annot_to_mich_type("nat", []), att.option_annot_to_mich_type(att.prim_annot_to_mich_type("nat", []), []), ["%keys"]),
...
export class Type_asset_key_1_iterable_big_map_option_nat {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./tests/contracts/type_asset_key_1_iterable_big_map_option_nat.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset_value(key: att.Option<att.Nat>): Promise<string | undefined> {
        if (this.address != undefined) {
            ...
            const raw_data = await ex.get_big_map_value(BigInt(att.Int.from_mich((storage as att.Mpair)?.args[0]).toString()), key.to_mich((x => { return x.to_mich(); })), my_asset_key_mich_type);
...
export const type_asset_key_1_iterable_big_map_option_nat = new Type_asset_key_1_iterable_big_map_option_nat();
