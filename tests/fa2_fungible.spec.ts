import { Bytes, Key, MichelineType, Nat, Option, Or, pair_to_mich, Signature, string_to_mich, mich_array_to_mich, pair_array_to_mich_type, prim_annot_to_mich_type, list_to_mich, list_to_mich_type, Address } from '@completium/archetype-ts-types'
...
import { balance_of_request, add_operator, remove_operator, template_fa2_fungible, gasless_param, operator_key, operator_param, transfer_destination, transfer_param, transfer_param_mich_type } from './contracts/bindings/template_fa2_fungible';
import { add, template_permits, permits_value, user_permit } from './contracts/bindings/template_permits';
...
describe('[FA2 fungible] Contracts deployment', async () => {
  it('Permits contract deployment should succeed', async () => {
    await template_permits.deploy(alice.get_address(), { as: alice })
  });
  it('FA2 fungible contract deployment should succeed', async () => {
    await template_fa2_fungible.deploy(alice.get_address(), template_permits.get_address(), { as: alice })
  });
});

describe('[FA2 fungible] Contract configuration', async () => {
  it("Add FA2 as permit consumer", async () => {
    await template_permits.manage_consumer(new add(template_fa2_fungible.get_address()), { as: alice })
  })
})

describe('[FA2 fungible] Minting', async () => {
  it('Mint tokens as owner for ourself should succeed', async () => {
    const balance_alice_before = await template_fa2_fungible.get_ledger_value(alice.get_address())
    assert(balance_alice_before?.equals(new Nat('123000000000000')), "Invalid amount")

    await template_fa2_fungible.mint(alice.get_address(), new Nat(1000), { as: alice })

    const balance_alice_after = await template_fa2_fungible.get_ledger_value(alice.get_address())
  ...
  it('Mint tokens as non owner for ourself should fail', async () => {
    await expect_to_fail(async () => {
      await template_fa2_fungible.mint(bob.get_address(), new Nat(1000), { as: bob })
    }, template_fa2_fungible.errors.INVALID_CALLER);
  });

  it('Mint tokens as non owner for someone else should fail', async () => {
    await expect_to_fail(async () => {
      await template_fa2_fungible.mint(carl.get_address(), new Nat(1000), { as: bob })
    }, template_fa2_fungible.errors.INVALID_CALLER);
  });

  it('Mint tokens as owner for someone else should succeed', async () => {
    const balance_carl_before = await template_fa2_fungible.get_ledger_value(carl.get_address())
    assert(balance_carl_before == undefined, "Invalid amount")

    await template_fa2_fungible.mint(carl.get_address(), new Nat(1000), { as: alice })

    const balance_carl_after = await template_fa2_fungible.get_ledger_value(carl.get_address())
    assert(balance_carl_after?.equals(new Nat(1000)), "Invalid amount")
  });

  it('Mint token for user 1', async () => {
    const balance_user1_before = await template_fa2_fungible.get_ledger_value(user1.get_address())
    assert(balance_user1_before == undefined, "Invalid amount")

    await template_fa2_fungible.mint(user1.get_address(), new Nat(1), { as: alice })

    const balance_user1_after = await template_fa2_fungible.get_ledger_value(user1.get_address())
...
describe('[FA2 fungible] Update operators', async () => {
  it('Add an operator for ourself should succeed', async () => {
    const op_key = new operator_key(template_fa2_fungible.get_address(), token_id, alice.get_address())
    const has_operator_before = await template_fa2_fungible.has_operator_value(op_key)
    assert(has_operator_before == false)
    await template_fa2_fungible.update_operators([
      new add_operator(new operator_param(alice.get_address(), template_fa2_fungible.get_address(), token_id))
    ], { as: alice })
    const has_operator_after = await template_fa2_fungible.has_operator_value(op_key)
  ...
  it('Remove an existing operator for another user should fail', async () => {
    await expect_to_fail(async () => {
      await template_fa2_fungible.update_operators([
        new remove_operator(new operator_param(alice.get_address(), template_fa2_fungible.get_address(), token_id))
      ], { as: bob })
    }, template_fa2_fungible.errors.FA2_NOT_OWNER);
  });

  it('Add operator for another user should fail', async () => {
    await expect_to_fail(async () => {
      await template_fa2_fungible.update_operators([
        new add_operator(new operator_param(bob.get_address(), template_fa2_fungible.get_address(), token_id))
      ], { as: alice });
    }, template_fa2_fungible.errors.FA2_NOT_OWNER);
  });
...
describe('[FA2 fungible] Add permit', async () => {
  it('Add a permit with the wrong signature should fail', async () => {
    const alice_permit_counter = (await template_permits.get_permits_value(alice.get_address()))?.counter
    ...
    const permit_data = get_transfer_permit_data(
      ...
      template_permits.get_address(),
      alice_permit_counter);
    await expect_to_fail(async () => {
      await template_permits.permit(new Key(alice.pubk), wrong_sig, blake2b(packed_transfer_params), { as: bob })
  ...
  it('Add a permit with the wrong hash should fail', async () => {
    ...
    await expect_to_fail(async () => {
      ...
      await template_permits.permit(new Key(alice.pubk), sig, blake2b(wrong_packed_transfer_params), { as: bob })
  ...
  it('Add a permit with the wrong public key should fail', async () => {
    ...
    await expect_to_fail(async () => {
      const sig = await alice.sign(blake2b(permit_data))
      await template_permits.permit(new Key(bob.pubk), sig, blake2b(packed_transfer_params), { as: bob })
    }, get_missigned_error(permit_data));
  });
  ...
  it('Add a permit with the good hash, signature and public key should succeed', async () => {
    ...
    await template_permits.permit(new Key(alice.pubk), sig, blake2b(packed_transfer_params), { as: bob })
  ...
  it('Add a duplicated permit should fail', async () => {
    const initial_permit = await template_permits.get_permits_value(alice.get_address())
...
describe('[FA2 fungible] Transfers', async () => {
  it('Transfer simple amount of token', async () => {
    const balance_user1_before = await template_fa2_fungible.get_ledger_value(user1.get_address())
    const balance_user2_before = await template_fa2_fungible.get_ledger_value(user2.get_address())
    ...
    await template_fa2_fungible.transfer([new transfer_param(
      user1.get_address(),
    ...
    const balance_user1_after = await template_fa2_fungible.get_ledger_value(user1.get_address())
    const balance_user2_after = await template_fa2_fungible.get_ledger_value(user2.get_address())
  ...
  it('Transfer a token from another user without a permit or an operator should fail', async () => {
    ...
    }, template_fa2_fungible.errors.FA2_NOT_OPERATOR);
  ...
  it('Transfer more tokens than owned should fail', async () => {
    ...
    }, template_fa2_fungible.errors.FA2_INSUFFICIENT_BALANCE);
...
describe('[FA2 fungible] Transfers gasless ', async () => {
  it('Transfer gasless simple amount of token', async () => {
    ...
    const permit = await template_permits.get_permits_value(user1.get_address())
    ...
    await template_fa2_fungible.transfer_gasless([
  ...
  it('Transfer a token from another user with wrong permit should fail', async () => {
    const amount = new Nat(1);
    const permit = await template_permits.get_permits_value(user2.get_address())
    const counter = permit?.counter
    const balance_user1_before = await template_fa2_fungible.get_ledger_value(user1.get_address())
    const balance_user2_before = await template_fa2_fungible.get_ledger_value(user2.get_address())
    assert(balance_user1_before == undefined, "Invalid amount user1")
  ...
  it('Transfer tokens gasless with from different signer should fail', async () => {
    await template_fa2_fungible.mint(user4.get_address(), new Nat(1), { as: alice })

    const amount = new Nat(1);
    const permit = await template_permits.get_permits_value(user4.get_address())
    const counter = permit?.counter
    const balance_user4_before = await template_fa2_fungible.get_ledger_value(user4.get_address())
    const balance_user5_before = await template_fa2_fungible.get_ledger_value(user5.get_address())
    ...
    }, template_fa2_fungible.errors.SIGNER_NOT_FROM);
  });
...
describe('[FA2 fungible] Transfers one-step ', async () => {
  it('Transfer one-step simple amount of token', async () => {
    ...
    await template_fa2_fungible.permit_transfer(tps, lpermit, { as: user3 })
  ...
  it('Transfer one-step without permit (None value) should succed', async () => {
    ...
    await template_fa2_fungible.permit_transfer(tps, Option.None<[Key, Signature]>(), { as: user2 })
...
describe('[FA2 fungible] Consume permit', async () => {
  ...
  it('Simple transfer with permit', async () => {
    ...
    await template_permits.permit(user1.get_public_key(), sig, blake2b(packed_transfer_params), { as: bob })

    const permit_after = await template_permits.get_permits_value(user1.get_address())
    assert(permit_after?.user_permits.length == 1, "Invalid user permits")

    await template_fa2_fungible.transfer(tps, { as: user3 })
  ...
  it('Set expiry for an existing permit with too big value should fail', async () => {
    ...
    await template_permits.permit(user2.get_public_key(), sig, blake2b(packed_transfer_params), { as: user2 })
  ...
  it('Set expiry with a correct value should succeed', async () => {
    await template_fa2_fungible.mint(user2.get_address(), new Nat(1), { as: alice })
    ...
    await template_permits.set_expiry(Option.Some(expiry), Option.Some(blake2b(packed_transfer_params)), { as: user2 })

    //set_mockup_now(new Date(now.setSeconds(now.getSeconds() + expiry.to_big_number().toNumber() + 10)))

    //await expect_to_fail(async () => {
    //  await template_fa2_fungible.transfer(tps, { as: user3 });
    //}, template_permits.errors.p7);
  ...
  it('Set expiry to 0 (permit gets deleted) should succeed', async () => {
    ...
    const permit = await template_permits.get_permits_value(carl.get_address())
    ...
    await template_permits.permit(carl.get_public_key(), sig, blake2b(packed_transfer_params), { as: carl })

    const added_permit = await template_permits.get_permits_value(carl.get_address())
    assert(added_permit?.equals(get_ref_user_permits(new Nat(1), packed_transfer_params, expiry, now)))

    await template_permits.set_expiry(Option.Some(new Nat(0)), Option.Some(blake2b(packed_transfer_params)), { as: carl })

    const final_permit = await template_permits.get_permits_value(carl.get_address())
...
describe('[FA2 fungible] Set metadata', async () => {
  it('Set metadata with empty content should succeed', async () => {
    const metadata_before = await template_fa2_fungible.get_metadata_value("key")
    assert(metadata_before == undefined);

    await template_fa2_fungible.set_metadata("key", Option.Some(new Bytes("")), { as: alice })

    const metadata_after = await template_fa2_fungible.get_metadata_value("key")
    assert(metadata_after?.equals(new Bytes("")));
  });

  it('Set metadata called by not owner should fail', async () => {
    await expect_to_fail(async () => {
      await template_fa2_fungible.set_metadata("key", Option.Some(new Bytes("")), { as: bob })
    }, template_fa2_fungible.errors.INVALID_CALLER);
  });
  ...
  it('Set metadata with valid content should succeed', async () => {
    ...
    await template_fa2_fungible.set_metadata("key", Option.Some(data), { as: alice })
...
describe('[FA2 fungible] Burn', async () => {
  it('Burn token should succeed', async () => {
    ...
    await template_fa2_fungible.burn(new Nat(2), { as: user1 });
  ...
  it('Burn without tokens should fail', async () => {
    await expect_to_fail(async () => {
      await template_fa2_fungible.burn(new Nat(1), { as: user1 });
    }, template_fa2_fungible.errors.FA2_INSUFFICIENT_BALANCE);
  });

  it('Burn tokens with a partial amount of tokens should succeed', async () => {
    const amount = new Nat(500)
    const balance_user1_before = await template_fa2_fungible.get_ledger_value(carl.get_address())

    await template_fa2_fungible.burn(amount, { as: carl });

    const balance_user1_after = await template_fa2_fungible.get_ledger_value(carl.get_address())
  ...
  it('Burn tokens with more tokens owned should fail', async () => {
    ...
    await expect_to_fail(async () => {
      await template_fa2_fungible.burn(new Nat(1000), { as: carl });
    }, template_fa2_fungible.errors.FA2_INSUFFICIENT_BALANCE);
...
describe('[FA2 fungible] Pause', async () => {
  it('Set FA2 on pause should succeed', async () => {
    await template_fa2_fungible.pause({ as: alice });
    const is_paused = await template_fa2_fungible.get_paused()
    assert(is_paused);
  });
  it('Set Permits on pause should succeed', async () => {
    await template_permits.pause({ as: alice });
    const is_paused = await template_fa2_fungible.get_paused()
    assert(is_paused);
  });
  ...
  it('Minting is not possible when contract is paused should fail', async () => {
    await expect_to_fail(async () => {
      await template_fa2_fungible.mint(alice.get_address(), new Nat(1000), { as: alice })
    }, template_fa2_fungible.errors.CONTRACT_PAUSED);
  });
  ...
  it('Add permit is not possible when contract is paused should fail', async () => {
    ...
    await expect_to_fail(async () => {
      await template_permits.permit(alice.get_public_key(), sig, packed_transfer_params, { as: alice });
    }, template_fa2_fungible.errors.CONTRACT_PAUSED);
  });
  ...
  it('Set expiry is not possible when contract is paused should fail', async () => {
    ...
    await expect_to_fail(async () => {
      await template_permits.set_expiry(Option.Some(new Nat(0)), Option.Some(blake2b(packed_transfer_params)), { as: alice })
    }, template_fa2_fungible.errors.CONTRACT_PAUSED);
  });

  it('Burn is not possible when contract is paused should fail', async () => {
    await expect_to_fail(async () => {
      await template_fa2_fungible.burn(new Nat(1), { as: alice })
    }, template_fa2_fungible.errors.CONTRACT_PAUSED);
  });

  it('Unpause by not owner should fail', async () => {
    await expect_to_fail(async () => {
      await template_fa2_fungible.unpause({ as: bob });
    }, template_fa2_fungible.errors.INVALID_CALLER);
  });

  it('Unpause by owner should succeed', async () => {
    await template_fa2_fungible.unpause({ as: alice });
    await template_permits.unpause({ as: alice });
  });
});
...
describe('[FA2 fungible] Transfer ownership', async () => {
  ...
  it('Transfer ownership when contract is paused should succeed', async () => {
    const owner = await template_fa2_fungible.get_owner()
    assert(owner.equals(alice.get_address()));
    await template_fa2_fungible.declare_ownership(alice.get_address(), { as: alice });
    const new_owner = await template_fa2_fungible.get_owner()
    assert(owner.equals(new_owner));
  });

  it('Transfer ownership as non owner should fail', async () => {
    await expect_to_fail(async () => {
      await template_fa2_fungible.declare_ownership(bob.get_address(), { as: bob });
    }, template_fa2_fungible.errors.INVALID_CALLER);
  });
  ...
  it('Transfer ownership as owner should succeed', async () => {
    const owner = await template_fa2_fungible.get_owner()
    assert(owner.equals(alice.get_address()));
    await template_fa2_fungible.declare_ownership(bob.get_address(), { as: alice })
    await template_fa2_fungible.claim_ownership({ as: bob });
    const new_owner = await template_fa2_fungible.get_owner()
...
describe('[FA2 fungible] Balance of', async () => {
  ...
  it('Simple balance of', async () => {
    const balance_alice = await template_fa2_fungible.get_ledger_value(alice.get_address())

    const res = await template_fa2_fungible.balance_of([new balance_of_request(alice.get_address(), token_id)], { as: alice })
  ...
  it('Call balance of with unknown address', async () => {
    const res = await template_fa2_fungible.balance_of([new balance_of_request(user5.get_address(), token_id)], { as: alice })
  ...
  it('Unknown token_id should fail', async () => {
    ...
    await expect_to_fail(async () => {
      await template_fa2_fungible.balance_of([new balance_of_request(alice.get_address(), fake_token)], { as: alice })
    }, template_fa2_fungible.errors.FA2_TOKEN_UNDEFINED)
  });
});
